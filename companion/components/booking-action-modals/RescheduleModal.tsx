...
export function RescheduleModal({
  ...
  useEffect(() => {
    if (visible && currentStartTime) {
      setDate(formatDateForInput(currentStartTime));
      setTime(formatTimeForInput(currentStartTime));
      setReason("");
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSubmit(date.trim(), time.trim(), reason.trim() || undefined);
    } catch (error) {
  ...
  const canSubmit = date.trim().length > 0 && time.trim().length > 0 && !isLoading;
    ...
    <FullScreenModal visible={visible} animationType="slide" onRequestClose={handleClose}>
      <View className="flex-1 bg-[#F2F2F7]" style={{ paddingTop: insets.top }}>
        ...
        <View className="flex-row items-center justify-between border-b border-gray-200 bg-white px-4 py-3">
          ...
          <Text className="text-[17px] font-semibold text-[#000]">Reschedule</Text>
          <TouchableOpacity
          ...
          >
            {isLoading ? (
              ...
              <Text
                className={`text-base font-semibold ${canSubmit ? "text-[#007AFF]" : "text-gray-300"}`}
              >
                Save
              </Text>
            )}
