...
export default class OrganizerCancelledEmail extends OrganizerScheduledEmail {
  protected getNodeMailerPayload(): Record<string, unknown> {
    ...
    return {
      ...
      to: toAddresses.join(","),
      subject: `${this.calEvent.organizer.language("event_cancelled_subject", {
  ...
  protected getTextBody(): string {
    return `
${this.calEvent.organizer.language("event_request_cancelled")}
...
`.replace(/(<([^>]+)>)/gi, "");
  ...
  protected getHtmlBody(): string {
    const headerContent = this.calEvent.organizer.language("event_cancelled_subject", {
      eventType: this.calEvent.type,
      name: this.calEvent.attendees[0].name,
      date: `${this.getOrganizerStart().format("h:mma")} - ${this.getOrganizerEnd().format(
        "h:mma"
