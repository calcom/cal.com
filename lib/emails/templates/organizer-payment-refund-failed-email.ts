...
export default class OrganizerPaymentRefundFailedEmail extends OrganizerScheduledEmail {
  protected getNodeMailerPayload(): Record<string, unknown> {
    ...
    return {
      ...
      to: toAddresses.join(","),
      subject: `${this.calEvent.organizer.language("refund_failed_subject", {
  ...
  protected getTextBody(): string {
    return `
${this.calEvent.organizer.language("a_refund_failed")}
${this.calEvent.organizer.language("check_with_provider_and_user", { user: this.calEvent.attendees[0].name })}
${
  this.calEvent.paymentInfo &&
  this.calEvent.organizer.language("error_message", { errorMessage: this.calEvent.paymentInfo.reason })
}
${this.getWhat()}
${this.getWhen()}
${this.getLocation()}
...
`.replace(/(<([^>]+)>)/gi, "");
  ...
  protected getHtmlBody(): string {
    const headerContent = this.calEvent.organizer.language("refund_failed_subject", {
      eventType: this.calEvent.type,
      name: this.calEvent.attendees[0].name,
      date: `${this.getOrganizerStart().format("h:mma")} - ${this.getOrganizerEnd().format(
        "h:mma"
                      ...
                      <tbody>
                        <tr>
                          <td align="center" style="font-size:0px;padding:10px 25px;padding-top:24px;padding-bottom:0px;word-break:break-word;">
                            <div style="font-family:Roboto, Helvetica, sans-serif;font-size:24px;font-weight:700;line-height:24px;text-align:center;color:#292929;">${this.calEvent.organizer.language(
                              "a_refund_failed"
                            )}</div>
                          </td>
                        </tr>
                        <tr>
                          <td align="center" style="font-size:0px;padding:10px 25px;word-break:break-word;">
                            <div style="font-family:Roboto, Helvetica, sans-serif;font-size:16px;font-weight:400;line-height:24px;text-align:center;color:#494949;">${this.calEvent.organizer.language(
