...
export default class OrganizerRequestReminderEmail extends OrganizerScheduledEmail {
  protected getNodeMailerPayload(): Record<string, unknown> {
    ...
    return {
      ...
      to: toAddresses.join(","),
      subject: `${this.calEvent.organizer.language("event_awaiting_approval_subject", {
  ...
  protected getTextBody(): string {
    return `
${this.calEvent.organizer.language("event_still_awaiting_approval")}
${this.calEvent.organizer.language("someone_requested_an_event")}
${this.getWhat()}
${this.getWhen()}
${this.getLocation()}
${this.getAdditionalNotes()}
${this.calEvent.organizer.language("confirm_or_reject_request")}
${process.env.BASE_URL} + "/bookings/upcoming"
`.replace(/(<([^>]+)>)/gi, "");
  }

  protected getHtmlBody(): string {
    const headerContent = this.calEvent.organizer.language("event_awaiting_approval_subject", {
      eventType: this.calEvent.type,
      name: this.calEvent.attendees[0].name,
      date: `${this.getOrganizerStart().format("h:mma")} - ${this.getOrganizerEnd().format(
        "h:mma"
  ...
  protected getManageLink(): string {
    const manageText = this.calEvent.organizer.language("confirm_or_reject_request");
