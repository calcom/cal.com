...
export default class OrganizerRequestReminderEmail extends OrganizerScheduledEmail {
  protected getNodeMailerPayload(): Record<string, unknown> {
    ...
    return {
      ...
      to: toAddresses.join(","),
      subject: `${this.calEvent.organizerLanguage("event_awaiting_approval_subject", {
  ...
  protected getTextBody(): string {
    return `
${this.calEvent.organizerLanguage("event_still_awaiting_approval")}
${this.calEvent.organizerLanguage("someone_requested_an_event")}
${this.getWhat()}
${this.getWhen()}
${this.getLocation()}
${this.getAdditionalNotes()}
${this.calEvent.organizerLanguage("confirm_or_reject_request")}
${process.env.BASE_URL} + "/bookings/upcoming"
`.replace(/(<([^>]+)>)/gi, "");
  }

  protected getHtmlBody(): string {
    const headerContent = this.calEvent.organizerLanguage("event_awaiting_approval_subject", {
      eventType: this.calEvent.type,
      name: this.calEvent.attendees[0].name,
      date: `${this.getOrganizerStart().format("h:mma")} - ${this.getOrganizerEnd().format(
        "h:mma"
  ...
  protected getManageLink(): string {
    const manageText = this.calEvent.organizerLanguage("confirm_or_reject_request");
