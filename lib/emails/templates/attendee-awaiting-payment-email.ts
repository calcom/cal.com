...
export default class AttendeeAwaitingPaymentEmail extends AttendeeScheduledEmail {
  protected getNodeMailerPayload(): Record<string, unknown> {
    return {
      ...
      subject: `${this.calEvent.attendees[0].language("awaiting_payment_subject", {
  ...
  protected getTextBody(): string {
    return `
${this.calEvent.attendees[0].language("meeting_awaiting_payment")}
...
`.replace(/(<([^>]+)>)/gi, "");
  ...
  protected getHtmlBody(): string {
    const headerContent = this.calEvent.attendees[0].language("awaiting_payment_subject", {
      eventType: this.calEvent.type,
      name: this.calEvent.team?.name || this.calEvent.organizer.name,
      date: `${this.getInviteeStart().format("h:mma")} - ${this.getInviteeEnd().format(
        "h:mma"
  ...
  protected getManageLink(): string {
    const manageText = this.calEvent.attendees[0].language("pay_now");
