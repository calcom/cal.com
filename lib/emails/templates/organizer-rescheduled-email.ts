...
export default class OrganizerRescheduledEmail extends OrganizerScheduledEmail {
  protected getNodeMailerPayload(): Record<string, unknown> {
    ...
    return {
      ...
      to: toAddresses.join(","),
      subject: `${this.calEvent.organizerLanguage("rescheduled_event_type_subject", {
  ...
  protected getTextBody(): string {
    return `
${this.calEvent.organizerLanguage("event_has_been_rescheduled")}
${this.calEvent.organizerLanguage("emailed_you_and_any_other_attendees")}
${this.getWhat()}
${this.getWhen()}
${this.getLocation()}
${this.getAdditionalNotes()}
${this.calEvent.organizerLanguage("need_to_reschedule_or_cancel")}
${getCancelLink(this.calEvent)}
`.replace(/(<([^>]+)>)/gi, "");
  }

  protected getHtmlBody(): string {
    const headerContent = this.calEvent.organizerLanguage("rescheduled_event_type_subject", {
      eventType: this.calEvent.type,
      name: this.calEvent.attendees[0].name,
      date: `${this.getOrganizerStart().format("h:mma")} - ${this.getOrganizerEnd().format(
        "h:mma"
