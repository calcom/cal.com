...
export default class AttendeeDeclinedEmail extends AttendeeScheduledEmail {
  protected getNodeMailerPayload(): Record<string, unknown> {
    return {
      ...
      subject: `${this.calEvent.attendees[0].language("event_declined_subject", {
  ...
  protected getTextBody(): string {
    return `
${this.calEvent.attendees[0].language("event_request_declined")}
...
`.replace(/(<([^>]+)>)/gi, "");
  ...
  protected getHtmlBody(): string {
    const headerContent = this.calEvent.attendees[0].language("event_declined_subject", {
      eventType: this.calEvent.type,
      name: this.calEvent.team?.name || this.calEvent.organizer.name,
      date: `${this.getInviteeStart().format("h:mma")} - ${this.getInviteeEnd().format(
        "h:mma"
