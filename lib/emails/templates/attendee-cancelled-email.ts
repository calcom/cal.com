...
export default class AttendeeCancelledEmail extends AttendeeScheduledEmail {
  protected getNodeMailerPayload(): Record<string, unknown> {
    return {
      ...
      subject: `${this.calEvent.attendeesLanguage("event_cancelled_subject", {
  ...
  protected getTextBody(): string {
    return `
${this.calEvent.attendeesLanguage("event_request_cancelled")}
...
`.replace(/(<([^>]+)>)/gi, "");
  ...
  protected getHtmlBody(): string {
    const headerContent = this.calEvent.attendeesLanguage("event_cancelled_subject", {
      eventType: this.calEvent.type,
      name: this.calEvent.team?.name || this.calEvent.organizer.name,
      date: `${this.getInviteeStart().format("h:mma")} - ${this.getInviteeEnd().format(
        "h:mma"
