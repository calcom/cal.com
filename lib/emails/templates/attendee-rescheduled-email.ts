...
export default class AttendeeRescheduledEmail extends AttendeeScheduledEmail {
  protected getNodeMailerPayload(): Record<string, unknown> {
    return {
      ...
      subject: `${this.calEvent.attendeesLanguage("rescheduled_event_type_subject", {
  ...
  protected getTextBody(): string {
    ...
    if (this.attendee === this.calEvent.attendees[0]) {
      return `
  ${this.calEvent.attendeesLanguage("event_has_been_rescheduled")}
  ${this.calEvent.attendeesLanguage("emailed_you_and_any_other_attendees")}
  ${this.getWhat()}
  ${this.getWhen()}
  ${this.getLocation()}
  ${this.getAdditionalNotes()}
  ${this.calEvent.attendeesLanguage("need_to_reschedule_or_cancel")}
...
`.replace(/(<([^>]+)>)/gi, "");
  ...
  protected getHtmlBody(): string {
    const headerContent = this.calEvent.attendeesLanguage("rescheduled_event_type_subject", {
      eventType: this.calEvent.type,
      name: this.calEvent.team?.name || this.calEvent.organizer.name,
      date: `${this.getInviteeStart().format("h:mma")} - ${this.getInviteeEnd().format(
        "h:mma"
