...
export default class AttendeeRescheduledEmail extends AttendeeScheduledEmail {
  protected getNodeMailerPayload(): Record<string, unknown> {
    return {
      ...
      subject: `${this.calEvent.attendees[0].language("rescheduled_event_type_subject", {
  ...
  protected getTextBody(): string {
    ...
    if (this.attendee === this.calEvent.attendees[0]) {
      return `
  ${this.calEvent.attendees[0].language("event_has_been_rescheduled")}
  ${this.calEvent.attendees[0].language("emailed_you_and_any_other_attendees")}
  ${this.getWhat()}
  ${this.getWhen()}
  ${this.getLocation()}
  ${this.getAdditionalNotes()}
  ${this.calEvent.attendees[0].language("need_to_reschedule_or_cancel")}
...
`.replace(/(<([^>]+)>)/gi, "");
  ...
  protected getHtmlBody(): string {
    const headerContent = this.calEvent.attendees[0].language("rescheduled_event_type_subject", {
      eventType: this.calEvent.type,
      name: this.calEvent.team?.name || this.calEvent.organizer.name,
      date: `${this.getInviteeStart().format("h:mma")} - ${this.getInviteeEnd().format(
        "h:mma"
