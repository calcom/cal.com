...
export default class EventManager {
  ...
  constructor(user: EventManagerUser) {
    ...
    //for  Daily.co video, temporarily pushes a installed apps for the daily-video-client
  ...
  /**
   * Creates event entries for all calendar integrations given in the installed apps.
   * When noMail is true, no mails will be sent. This is used when the event is
   * a video meeting because then the mail containing the video installed apps will be
   ...
   * installed apps for that - if existing.
   * @param event
   * @private
   */
  ...
  private createVideoEvent(event: Ensure<CalendarEvent, "language">): Promise<EventResult> {
    ...
    if (installedApp) {
      ...
      return Promise.reject("No suitable installed apps given for the requested integration name.");
    }
  }

  /**
   * Updates the event entries for all calendar integrations given in the installed apps.
   * When noMail is true, no mails will be sent. This is used when the event is
   * a video meeting because then the mail containing the video installed apps will be
   * more important than the mails created for these bare calendar events.
   *
