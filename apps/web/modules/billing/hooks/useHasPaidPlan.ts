...
export function useHasPaidPlan() {
  ...
  const { data, isPending: isPendingTeamQuery } =
    trpc.viewer.teams.hasTeamPlan.useQuery();

  const { data: user, isPending: isPendingUserQuery } =
    trpc.viewer.me.get.useQuery();
...
export function useHasTeamPlan() {
  const { data: hasTeamPlan, isPending } =
    trpc.viewer.teams.hasTeamPlan.useQuery();
...
export function useHasActiveTeamPlan() {
  if (IS_SELF_HOSTED)
    return { isPending: false, hasActiveTeamPlan: true, isTrial: false };

  const { data, isPending } = trpc.viewer.teams.hasActiveTeamPlan.useQuery();

  return {
    isPending,
    hasActiveTeamPlan: !!data?.isActive,
