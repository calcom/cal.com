...
} from "@calcom/features/embed/EventTypeEmbed";
...
interface EventTypeReorderItemProps {
  type: InfiniteEventType;
  children: React.ReactNode;
}

function EventTypeReorderItem({ type, children }: EventTypeReorderItemProps) {
    ...
    >
      <div className="hover:bg-cal-muted flex w-full items-center justify-between transition">
        <div className="group flex w-full max-w-full items-center justify-between overflow-hidden px-4 py-4 sm:px-6">
          {/* Drag handle - always visible on desktop, hidden on mobile */}
          <div
            className="hover:bg-subtle text-muted mr-2 hidden cursor-grab rounded p-1.5 active:cursor-grabbing sm:block"
...
export const InfiniteEventTypeList = ({
  ...
  const [flattenedEventTypes, setFlattenedEventTypes] = useState<
    InfiniteEventType[]
  >(() => pages?.flatMap((page) => page.eventTypes) ?? []);
  ...
  // Debounce timer ref for mutation calls
  const debounceTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const pendingOrderRef = useRef<InfiniteEventType[] | null>(null);

  // Store stable refs for dependencies to avoid recreating callbacks
  ...
  }, [utils, debouncedSearchTerm, group, mutation]);

  // Utility to chunk array for re-pagination
  ...
  // Persist the order to the server (using refs to avoid dependency issues)
  const persistOrderRef =
    useRef<(newOrder: InfiniteEventType[]) => Promise<void>>();

  persistOrderRef.current = async (newOrder: InfiniteEventType[]) => {
    ...
    await currentUtils.viewer.eventTypes.getEventTypesFromGroup.cancel();

    const previousValue =
      currentUtils.viewer.eventTypes.getEventTypesFromGroup.getInfiniteData({
        limit: LIMIT,
    ...
    if (previousValue) {
      const repaginated = chunkArray(newOrder, LIMIT);

      currentUtils.viewer.eventTypes.getEventTypesFromGroup.setInfiniteData(
        {
          limit: LIMIT,
          searchQuery: currentSearchTerm,
          group: {
        ...
        (data) => {
          ...
          return {
            ...data,
            pages: repaginated.map((chunk, idx) => ({
              eventTypes: chunk,
              nextCursor: data.pages[idx]?.nextCursor ?? null,
            })),
          };
        }
      );
    ...
    currentMutation.mutate({
      ids: newOrder.map((type) => type.id),
    });
  };

  // Handle drag-and-drop reordering with debounced mutation
  const handleReorder = useCallback((newOrder: InfiniteEventType[]) => {
    // Update local state immediately for responsive UI
    setFlattenedEventTypes(newOrder);
    ...
    // Set new debounce timer to persist after 500ms of inactivity
    debounceTimerRef.current = setTimeout(() => {
      const orderToPersist = pendingOrderRef.current;
      if (orderToPersist && persistOrderRef.current) {
        pendingOrderRef.current = null;
        debounceTimerRef.current = null;
        persistOrderRef.current(orderToPersist);
      }
    }, 500);
  }, []);

  // Cleanup: persist any pending order on unmount
  useEffect(() => {
    return () => {
      if (debounceTimerRef.current) {
        clearTimeout(debounceTimerRef.current);
      ...
      if (pendingOrderRef.current && persistOrderRef.current) {
        const orderToPersist = pendingOrderRef.current;
        pendingOrderRef.current = null;
        persistOrderRef.current(orderToPersist);
      }
  ...
  return (
    <div className="bg-default border-subtle flex flex-col overflow-hidden rounded-md border">
      <Reorder.Group
        as="ul"
        axis="y"
        values={flattenedEventTypes}
        onReorder={handleReorder}
        className="divide-subtle static! w-full divide-y"
        data-testid="event-types"
      >
        {flattenedEventTypes.map((type) => {
          ...
          const isChildrenManagedEventType =
            ...
            type.schedulingType !== SchedulingType.MANAGED;
          return (
            <EventTypeReorderItem key={type.id} type={type}>
