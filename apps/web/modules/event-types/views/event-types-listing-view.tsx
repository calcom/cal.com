...
interface SortableEventTypeItemProps {
  ...
  privateLinkCopyIndices: Record<string, number>;
  setPrivateLinkCopyIndices: React.Dispatch<React.SetStateAction<Record<string, number>>>;
  isNativeShare: boolean;
  setHiddenMutation: ReturnType<typeof trpc.viewer.eventTypesHeavy.update.useMutation>;
  setDeleteDialogOpen: (open: boolean) => void;
  setDeleteDialogTypeId: (id: number) => void;
  setDeleteDialogSchedulingType: (type: SchedulingType | null) => void;
  openDuplicateModal: (eventType: InfiniteEventType, group: InfiniteEventTypeGroup) => void;
  router: ReturnType<typeof useRouter>;
  t: ReturnType<typeof useLocale>["t"];
  copyToClipboard: ReturnType<typeof useCopy>["copyToClipboard"];
}
...
function SortableEventTypeItem({
  ...
  lockedByOrg,
  userTimezone,
  privateLinkCopyIndices,
  setPrivateLinkCopyIndices,
  isNativeShare,
  setHiddenMutation,
  setDeleteDialogOpen,
  setDeleteDialogTypeId,
...
export const InfiniteEventTypeList = ({
  ...
  const setHiddenMutation = trpc.viewer.eventTypesHeavy.update.useMutation({
    ...
    const handleDragEnd = useCallback(
      (event: DragEndEvent) => {
        ...
        const oldIndex = allEventTypes.findIndex((type) => type.id === active.id);
        const newIndex = allEventTypes.findIndex((type) => type.id === over.id);

        if (oldIndex === -1 || newIndex === -1) return;

        const reorderedEventTypes = arrayMove(allEventTypes, oldIndex, newIndex);
        ...
        utils.viewer.eventTypes.getEventTypesFromGroup.setInfiniteData(
          ...
          (data) => {
            ...
            const newPages = data.pages.map((page, pageIndex) => ({
              ...page,
              eventTypes: reorderedEventTypes.slice(pageIndex * LIMIT, (pageIndex + 1) * LIMIT),
            }));
      ...
      [pages, debouncedSearchTerm, group?.teamId, group?.parentId, utils, mutation]
    );
  ...
  const userTimezone = extractHostTimezone({
    ...
    const allEventTypes = pages.flatMap((page) => page.eventTypes);
    const eventTypeIds = allEventTypes.map((type) => type.id);
            ...
            <ul className="divide-subtle static! w-full divide-y" data-testid="event-types">
              {allEventTypes.map((type) => (
                <SortableEventTypeItem
                  ...
                  readOnly={readOnly}
                  bookerUrl={bookerUrl}
                  lockedByOrg={lockedByOrg}
                  userTimezone={userTimezone}
                  privateLinkCopyIndices={privateLinkCopyIndices}
                  setPrivateLinkCopyIndices={setPrivateLinkCopyIndices}
                  isNativeShare={isNativeShare}
                  setHiddenMutation={setHiddenMutation}
                  setDeleteDialogOpen={setDeleteDialogOpen}
                  setDeleteDialogTypeId={setDeleteDialogTypeId}
                  setDeleteDialogSchedulingType={setDeleteDialogSchedulingType}
