...
interface DynamicLinkDialogProps {
  isOpen: boolean;
  onClose: () => void;
  memberSource: "org" | "team";
}

const MEMBERS_LIMIT = 20;

export function DynamicLinkDialog({
  isOpen,
  onClose,
  memberSource,
}: DynamicLinkDialogProps) {
  const { t } = useLocale();
  ...
  const baseUrl =
    memberSource === "org"
      ? orgBranding?.fullDomain ?? WEBSITE_URL
      : WEBSITE_URL;
  ...
  const orgQuery =
    trpc.viewer.organizations.listMembersMinimal.useInfiniteQuery(
      ...
      {
        enabled: isOpen && memberSource === "org",
  ...
  const teamQuery = trpc.viewer.teams.listMembersMinimal.useInfiniteQuery(
    ...
    {
      enabled: isOpen && memberSource === "team",
      getNextPageParam: (lastPage) => lastPage.nextCursor,
    }
  );

  const activeQuery = memberSource === "org" ? orgQuery : teamQuery;
        ...
        <div className="flex flex-col">
            ...
            {isFetching && allMembers.length === 0 ? (
              <div className="flex justify-center items-center h-full">
                <div className="flex flex-col gap-2 items-center">
                  <LoaderIcon className="w-5 h-5 animate-spin text-subtle" />
                    ...
                    >
                      ...
                      {isSelected && (
                        <div className="ml-auto">
                          <CheckIcon className="w-4 h-4 text-emphasis" />
                        </div>
                      )}
                    </label>
                  );
          ...
          {selectedUsers.length >= 2 ? (
            <div className="mt-4">
              ...
              <div className="flex gap-2 items-center p-2 mt-2 rounded-md border bg-muted">
                <a
                  href={dynamicLink}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 min-w-0 text-sm text-emphasis hover:underline"
                >
                  {renderDynamicLink()}
                </a>
                <div className="flex gap-1 shrink-0">
                  ...
                  <Button
                    variant="icon"
                    color="minimal"
                    CustomStartIcon={<ExternalLinkIcon className="w-4 h-4" />}
