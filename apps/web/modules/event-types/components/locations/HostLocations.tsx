...
const useHostLocationHandlers = (
  ...
  const handleLocationChange = (userId: number, location: HostLocation | null) => {
    // Use setHosts from context instead of form setValue for performance
    setHosts(serverHosts, hosts.map((h) => (h.userId === userId ? { ...h, location } : h)));
  };
...
export const HostLocations = ({ eventTypeId, locationOptions }: HostLocationsProps) => {
  ...
  const { handleToggle, handleLocationChange } = useHostLocationHandlers(formMethods, hosts, serverHosts, setHosts);
