...
export default function WorkflowStepContainer(props: WorkflowStepProps) {
  ...
  const addVariableEmailSubject = (variable: string) => {
    if (step) {
      ...
      form.setValue(`steps.${step.stepNumber - 1}.emailSubject`, subjectWithAddedVariable, { shouldDirty: true });
    }
  };
                  ...
                  <Select
                    ...
                    onChange={(val) => {
                      if (val) {
                        ...
                        form.clearErrors(`steps.${step.stepNumber - 1}.sendTo`);
                        form.setValue(`steps.${step.stepNumber - 1}.action`, val.value, { shouldDirty: true });
                        setUpdateTemplate(!updateTemplate);
                      }
                    }}
                ...
                <Controller
                  ...
                  render={() => (
                    <CheckboxField
                      ...
                      descriptionClassName="ml-0"
                      onChange={(e) =>
                        form.setValue(`steps.${step.stepNumber - 1}.numberRequired`, e.target.checked, { shouldDirty: true })
                      }
                    />
                  )}
                />
                    ...
                    <Select
                      ...
                      onChange={(val) => {
                        if (val) {
                          ...
                          form.setValue(`steps.${step.stepNumber - 1}.template`, value, { shouldDirty: true });
                          setUpdateTemplate(!updateTemplate);
                        }
                      }}
          ...
          {!isCalAIAction(form.getValues(`steps.${step.stepNumber - 1}.action`)) && (
            <div className="bg-cal-muted border-muted mt-3 rounded-2xl border p-3">
              ...
              <Editor
                ...
                setText={(text: string) => {
                  props.form.setValue(`steps.${step.stepNumber - 1}.reminderBody`, text, { shouldDirty: true });
                  ...
                  <Controller
                    ...
                    render={() => (
                      <CheckboxField
                        ...
                        descriptionClassName="ml-0"
                        onChange={(e) =>
                          form.setValue(`steps.${step.stepNumber - 1}.includeCalendarEvent`, e.target.checked, { shouldDirty: true })
                        }
                      />
                    )}
                  />
