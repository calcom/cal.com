...
import {
  ...
  DOCUMENT_CATEGORIES,
} from "./compliance/compliance-documents";
...
const ComplianceView = () => {
  ...
  const dpaDocuments = COMPLIANCE_DOCUMENTS.filter(
    (doc) => doc.category === DOCUMENT_CATEGORIES.DPA
  );
  const reportDocuments = COMPLIANCE_DOCUMENTS.filter(
    (doc) => doc.category === DOCUMENT_CATEGORIES.REPORTS
  );
  const otherDocuments = COMPLIANCE_DOCUMENTS.filter(
    (doc) => doc.category === DOCUMENT_CATEGORIES.OTHER
  );
