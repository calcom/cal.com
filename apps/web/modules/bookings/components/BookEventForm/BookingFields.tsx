...
export const BookingFields = ({
  ...
  const syncConsolidatedPhone = useMemo(() => {
    if (!consolidatedPhoneInfo) return null;
    return createPhoneSyncHandler(consolidatedPhoneInfo.fieldNames, setValue);
  }, [consolidatedPhoneInfo, setValue]);
  ...
  // Event-driven sync function for location field phone value
  const syncPhoneFieldsFromLocation = (locationValue: unknown) => {
        ...
        return (
          <FormBuilderField
            ...
            onValueChange={({ value }) => {
              ...
              if (field.name === SystemField.Enum.location) {
                syncPhoneFieldsFromLocation(value);
              }
            }}
          />
        );
