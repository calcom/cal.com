...
export const MostCancelledEventTypesTable = () => {
  const { t } = useLocale();
  const insightsBookingParams = useInsightsBookingParameters();
  const { data, isSuccess, isPending, isError } = trpc.viewer.insights.mostCancelledEventTypes.useQuery(
  ...
  return (
    <ChartCard title={t("most_cancelled_event_types")} isPending={isPending} isError={isError}>
      {isSuccess && data ? (
        <>
          <div>
            {data
