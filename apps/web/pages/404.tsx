...
export default function Custom404() {
    ...
    useEffect(() => {
        ...
        if (!isValidOrgDomain || !currentOrgDomain) {
            ...
            setUrl(`${WEBSITE_URL}/signup?callbackUrl=settings/organizations/new%3Fslug%3D${username.replace("/", "")}`);
        }
    }, []);
    ...
    if (isInsights) {
        return (<>
        <HeadSeo title="Feature is currently disabled" description={t("404_page_not_found")} nextSeoProps={{
                nofollow: true,
                noindex: true,
            }}/>
    ...
    return (<>
      <HeadSeo title={isSignup ? t("signup_requires") : t("404_page_not_found")} description={t("404_page_not_found")} nextSeoProps={{
            nofollow: true,
            noindex: true,
        }}/>
      <div className="bg-default min-h-screen px-4" data-testid="404-page">
        <main className="mx-auto max-w-xl pb-6 pt-16 sm:pt-24">
          {isSignup && process.env.NEXT_PUBLIC_WEBAPP_URL !== "https://app.cal.com" ? (<div>
              <div>
              ...
              <div className="mt-12">
                ...
                <ul role="list" className="mt-4">
                  <li className="border-2 border-green-500 px-4 py-2">
                    <a href="https://console.cal.com" className="relative flex items-start space-x-4 py-6 rtl:space-x-reverse">
                ...
                <ul role="list" className="border-subtle divide-subtle divide-y">
                  <li className="px-4 py-2">
                    <Link href="https://cal.com/self-hosting/installation" className="relative flex items-start space-x-4 py-6 rtl:space-x-reverse">
                      <div className="flex-shrink-0">
                  ...
                  <li className="px-4 py-2">
                    <a href={JOIN_SLACK} className="relative flex items-start space-x-4 py-6 rtl:space-x-reverse">
                      <div className="flex-shrink-0">
                        <span className="bg-muted flex h-12 w-12 items-center justify-center rounded-lg">
                ...
                <div className="mt-8">
                  <Link href={`${WEBSITE_URL}/enterprise`} className="hover:text-subtle text-emphasis text-base font-medium">
                    {t("contact_sales")}
                    <span aria-hidden="true"> &rarr;</span>
                  </Link>
                </div>
              ...
              <div className="text-center">
                ...
                {isSubpage && currentPageType !== pageType.TEAM ? (<span className="mt-2 inline-block text-lg ">
                    {t("check_spelling_mistakes_or_go_back")}
                  </span>) : isCalcom ? (<a target="_blank" href={url} className="mt-2 inline-block text-lg" rel="noreferrer">
                  ...
                  </a>) : (<span className="mt-2 inline-block text-lg">
                    {t(`404_the_${currentPageType.toLowerCase()}`)}{" "}
              ...
              <div className="mt-12">
                ...
                currentPageType === pageType.TEAM) && (<ul role="list" className="my-4">
                    <li className="border-2 border-green-500 px-4 py-2">
                      <a href={url} target="_blank" className="relative flex items-start space-x-4 py-6 rtl:space-x-reverse" rel="noreferrer">
                        <div className="flex-shrink-0">
                          <span className="flex h-12 w-12 items-center justify-center rounded-lg bg-green-50">
                            <Check className="h-6 w-6 text-green-500" aria-hidden="true"/>
                          </span>
                    ...
                    : ""}${username}${currentPageType === pageType.ORG ? `.${subdomainSuffix()}` : ""}`}</strong>
                              </span>
                            </span>
                          </h3>
                ...
                .map((link, linkIdx) => (<li key={linkIdx} className="px-4 py-2">
                        <Link href={link.href} className="relative flex items-start space-x-4 py-6 rtl:space-x-reverse">
                          <div className="flex-shrink-0">
                            <span className="bg-muted flex h-12 w-12 items-center justify-center rounded-lg">
