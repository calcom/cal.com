...
async function handler(req: NextApiRequest) {
  ...
  if (!req?.query?.email || !currentUser) {
    throw new HttpError({ statusCode: 400, message: "Booking failed User not found" });
  }

  // eslint-disable-next-line prefer-const
  let { recurringCount, noEmail, eventTypeSlug, eventTypeId, hasHashedBookingLink, language, ...reqBody } =
    extendedBookingCreateBody.parse(req.body);

  const singleSession = await prisma.eventType.findFirst({
    where: {
  ...
  if (!singleSession) {
    throw new HttpError({ statusCode: 400, message: "Booking failed Session not found" });
  }

  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  eventTypeId = singleSession?.id;
  eventTypeSlug = singleSession?.slug;
