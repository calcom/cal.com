  ...
  if (teamIds && teamIds.length > 0) {
    ...
    eventTypeGroups = teams.map((team) => ({
      ...
      eventTypes: team.eventTypes
        .map((item) => ({
          ...item,
          URL: `${CAL_URL}/${
            item.team ? `team/${(item.team as any).slug}` : (item?.users?.[0] as any)?.username
          }/${item.slug}`,
          selected: false,
          locations: item.locations as unknown as LocationObject[],
          bookingFields: eventTypeBookingFields.parse(item.bookingFields || []),
        }))
        .sort((eventTypeA, eventTypeB) => (eventTypeB as any).position - (eventTypeA as any).position),
    })) as any;
  } else {
    ...
    if (user) {
      eventTypeGroups.push({
        ...
        eventTypes: (user.eventTypes as any[])
          .map((item) => ({
            ...item,
            URL: `${CAL_URL}/${
              item.team ? `team/${(item.team as any).slug}` : (item?.users?.[0] as any)?.username
