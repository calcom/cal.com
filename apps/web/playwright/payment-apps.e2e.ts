...
import { gotoAndWaitForIdle, selectFirstAvailableTimeSlotNextMonth } from "./lib/testUtils";
...
test.describe("Payment app", () => {
  test("Should be able to edit alby price, currency", async ({ page, users }) => {
    ...
    await gotoAndWaitForIdle(page, `event-types/${paymentEvent?.id}?tabName=apps`);
    ...
    await gotoAndWaitForIdle(page, `${user.username}/${paymentEvent?.slug}`);
