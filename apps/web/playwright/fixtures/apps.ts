  ...
  return {
    ...
    installAnalyticsApp: async (app: string, eventTypeIds: number[]) => {
      ...
      if (eventTypeIds.length > 0) {
        await page
          .getByTestId(`select-event-type-${eventTypeIds[0]}`)
      ...
      if (app === "gtm") {
        ...
        for (let index = 0; index < eventTypeIds.length; index++) {
          await page
            .getByTestId("gtm-tracking-id-input")
            .nth(index)
            .fill("random-tracking-id");
        }
      }
    ...
    verifyConferencingApp: async (app: TApp) => {
      ...
      await expect(page.locator("[data-testid=where] ")).toContainText(
        app.label
      );
    },

    installConferencingAppNewFlow: async (
      app: TApp,
      eventTypeIds: number[]
    ) => {
    ...
    goToEventType: async (eventType: string) => {
      ...
      await expect(page.getByTestId("vertical-tab-basics")).toHaveAttribute(
        "aria-current",
        "page"
      );
    },
    goToAppsTab: async () => {
      ...
      await expect(
        page.getByTestId("vertical-tab-apps").first()
    ...
    verifyAppsInfo: async (activeApps: number) => {
      await expect(
        page.locator(`text=1 apps, ${activeApps} active`)
      ).toBeVisible();
    },
    verifyAppsInfoNew: async (app: string, eventTypeId: number) => {
      ...
      await expect(
        page.getByTestId("vertical-tab-apps").first()
      ).toHaveAttribute("aria-current", "page"); // fix the race condition
      await expect(
        page.locator(`[data-testid='${app}-app-switch'][data-state="checked"]`)
      ).toBeVisible();
    },
  };
}
