...
import type { Attendee, Booking, Prisma } from "@calcom/prisma/client";
import { BookingStatus } from "@calcom/prisma/enums";

import type { createBookingsFixture } from "./fixtures/bookings";
...
testBothBookers.describe("Reschedule Tests", async () => {
  ...
  test.describe("Seated attendees", () => {
    const registerSeatsForAttendees = async (
      ...
      return await bookings.update({
        ...
        data: {
          attendees: {
            update: attendees.map(
              (attendee, i): Prisma.AttendeeUpdateWithWhereUniqueWithoutBookingInput => ({
    ...
    test("First seated attendee should be able to reschedule a booking", async ({
      page,
      users,
      bookings,
    }) => {
      ...
      const attendeeSeatId = updatedBooking.seatsReferences[0].referenceUid;
      ...
      // The original booking is still present
