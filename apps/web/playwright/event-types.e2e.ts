...
testBothFutureAndLegacyRoutes.describe("Event Types tests", () => {
  test.describe("user", () => {
    ...
    test("Can add single use links", async ({ page }) => {
      await gotoFirstEventType(page);

      // Go to the advanced tab.
      await page.getByTestId("vertical-tab-event_advanced_tab_title").click();

      // Check if single-use link doesn't exist initially.
      await expect(page.locator("[data-testid=remove-single-use-link-0]")).toHaveCount(0);

      await addSingleUseLink(page);

      // Save the event.
      await saveEventType(page);

      // Check if the single-use link exists.
      await expect(page.locator("[data-testid=remove-single-use-link-0]")).toBeVisible();
    });
    test("Can remove single use links", async ({ page }) => {
      await gotoFirstEventType(page);
      ...
      // Wait for network idle before removing the link, otherwise save button doesn't activate.
      await page.getByTestId("toast-success").waitFor();
      await page.waitForLoadState("networkidle");

      await removeSingleUseLink(page);

      // Save the event.
      await saveEventType(page);

      // Check if single-use is removed correctly.
      await expect(page.locator("[data-testid=remove-single-use-link-0]")).toHaveCount(0);
    });
    test("Single use links should auto-destroy after use", async ({ page }) => {
      await gotoFirstEventType(page);
      ...
      const singleUseLink = await page.getByTestId("single-use-link-0").inputValue();
      await page.goto(singleUseLink);
      await selectFirstAvailableTimeSlotNextMonth(page);
      ...
      // Return back to the event-type's single-use link settings page.
      await page.goto("/event-types");
      await gotoFirstEventType(page);
      await page.getByTestId("vertical-tab-event_advanced_tab_title").click();

      // Check if single-use is removed correctly after usage.
      await expect(page.locator("[data-testid=remove-single-use-link-0]")).toHaveCount(0);
    });
  });
});

const addSingleUseLink = async (page: Page) => {
  // Add a single use link.
  await page.getByTestId("add-single-use-link-button").click();
};

const removeSingleUseLink = async (page: Page) => {
  await page.getByTestId("remove-single-use-link-0").click();
};
