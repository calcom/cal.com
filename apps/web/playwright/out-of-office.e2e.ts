...
test.describe("Out of office", () => {
  // NOTE: 2 tests below are known to fail due to the timezone issue in the past @see #16001
  const timeZoneEnumValues = Object.values(TimeZoneEnum);
  timeZoneEnumValues.forEach((timeZone) => {
    test(`[${timeZone}] User can create out of office entry`, async ({ page, users }) => {
      const user = await users.create({ name: "userOne", timeZone });
    ...
    test(`[${timeZone}] User can configure booking redirect`, async ({ page, users }) => {
      const user = await users.create({ name: "userOne", timeZone });
      const userTo = await users.create({ name: "userTwo", timeZone });
