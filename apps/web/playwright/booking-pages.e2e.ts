...
test.describe("Booking on different layouts", () => {
  ...
  test("Book on column layout", async ({ page }) => {
    ...
    // Wait for slot reservation to complete after clicking
    // This prevents race conditions where the slot appears unavailable
    // before the reservation completes and availability checks update
    const reservationPromise = page.waitForResponse(
      (response) =>
        response.url().includes("/api/trpc/viewer.slots.reserveSlot") &&
        response.status() === 200
    );
