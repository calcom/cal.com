...
import { Button, Dialog, DialogClose, DialogContent, DialogFooter, Dropdown, DropdownMenuContent, DropdownMenuItem, DropdownItem, DropdownMenuTrigger, List, Meta, showToast, SkeletonContainer, SkeletonText } from "@calcom/ui";
...
const ConferencingLayout = () => {
    ...
    const { data: defaultConferencingApp, isLoading: defaultConferencingAppLoading } = trpc.viewer.getUsersDefaultConferencingApp.useQuery();
    ...
    return (<div className="bg-default w-full sm:mx-0 xl:mt-0">
      <Meta title={t("conferencing")} description={t("conferencing_description")} CTA={<AddConferencingButton />}/>
      <List>
        {apps?.items &&
            apps.items
                ...
                return (<AppListCard description={app.description} title={app.title} logo={app.logo} key={app.title} isDefault={appIsDefault} // @TODO: Handle when a user doesnt have this value set
                 actions={<div>
                      <Dropdown>
                        ...
                        <DropdownMenuContent>
                          {!appIsDefault && (<DropdownMenuItem>
                              <DropdownItem type="button" color="secondary" StartIcon={Video} onClick={() => {
                                const locationType = getEventLocationTypeFromApp(app?.locationOption?.value ?? "");
                          ...
                          <DropdownMenuItem>
                            <DropdownItem type="button" color="destructive" disabled={app.isGlobal} StartIcon={Trash} onClick={() => {
      ...
      <Dialog open={deleteAppModal} onOpenChange={setDeleteAppModal}>
        <DialogContent title={t("Remove app")} description={t("are_you_sure_you_want_to_remove_this_app")} type="confirmation" Icon={AlertCircle}>
          <DialogFooter>
      ...
      {locationType && (<AppSetDefaultLinkDialog locationType={locationType} setLocationType={setLocationType} onSuccess={onSuccessCallback}/>)}
