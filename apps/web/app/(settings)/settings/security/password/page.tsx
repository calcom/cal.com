// This file has been sourced from: /Users/sean/Programming/cal.com/apps/web/pages/settings/security/password.tsx
...
const PasswordView = () => {
    ...
    return (<>
      ...
      {user && user.identityProvider !== IdentityProvider.CAL ? (<div>
          <div className="mt-6">
        ...
        </div>) : (<Form form={formMethods} handleSubmit={handleSubmit}>
          {formMethods.formState.errors.apiError && (<div className="pb-6">
          ...
          <div className="max-w-[38rem] sm:grid sm:grid-cols-2 sm:gap-x-4">
            ...
            <div>
              <PasswordField {...formMethods.register("newPassword", {
            minLength: {
          ...
          <div className="border-subtle mt-8 border-t py-8">
            <SettingsToggle title={t("session_timeout")} description={t("session_timeout_description")} checked={sessionTimeoutWatch !== undefined} data-testid="session-check" onCheckedChange={(e) => {
                if (!e) {
            ...
            {sessionTimeoutWatch && (<div className="mt-4 text-sm">
                <div className="flex items-center">
                  <p className="text-default ltr:mr-2 rtl:ml-2">{t("session_timeout_after")}</p>
                  <Select options={timeoutOptions} defaultValue={sessionTimeout
                    ? timeoutOptions.find((tmo) => tmo.value === sessionTimeout)
                    : timeoutOptions[1]} isSearchable={false} className="block h-[36px] !w-auto min-w-0 flex-none rounded-md text-sm" onChange={(event) => {
          ...
          <Button color="primary" className="mt-8" type="submit" onClick={() => formMethods.clearErrors("apiError")} disabled={isDisabled || passwordMutation.isLoading || sessionMutation.isLoading}>
            {t("update")}
          </Button>
        </Form>)}
    </>);
