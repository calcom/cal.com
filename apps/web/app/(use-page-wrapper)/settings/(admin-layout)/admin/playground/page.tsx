...
import { ArrowRightIcon, CalendarDaysIcon, CalendarIcon, ChartBarIcon, FilterIcon } from "lucide-react";
          ...
          {LINKS.map((link) => (
            <Link
              ...
              <div className="bg-emphasis/10 group-hover:bg-emphasis/20 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-md transition-colors">
                <link.Icon className="text-emphasis h-5 w-5" />
              </div>
              ...
              <ArrowRightIcon className="text-subtle group-hover:text-emphasis h-4 w-4 flex-shrink-0 transition-colors" />
            </Link>
          ))}
        </div>
      </div>
