...
/**
 * Server component that loads the "settings_organizations_roles" namespace translations for the user's locale.
 ...
 */
export default async function SettingsOrganizationsRolesNamespaceLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  const locale = (await getLocale(buildLegacyRequest(await headers(), await cookies()))) ?? "en";
  const translations = await loadTranslations(locale, "settings_organizations_roles");

  return (
    <I18nExtend translations={translations} ns="settings_organizations_roles">
      {children}
    </I18nExtend>
  );
}
