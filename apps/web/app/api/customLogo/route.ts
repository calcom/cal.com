...
const log = logger.getSubLogger({ prefix: ["[api/customLogo]"] });
...
const customLogoApiSchema = z.object({
...
const customLogoDefinitions: Record<CustomLogoType, CustomLogoTypeDefinition> = {
  logo: {
...
function isValidCustomLogoType(type: string): type is CustomLogoType {
  return type in customLogoDefinitions;
}
...
/**
 * This API endpoint serves custom team/organization logos with database queries and image processing.
 * For static favicon/app icons, use /api/logo instead.
 */
async function getHandler(request: NextRequest) {
  const searchParams = request.nextUrl.searchParams;
  const parsedQuery = customLogoApiSchema.parse(Object.fromEntries(searchParams.entries()));
  ...
  const type: CustomLogoType =
    parsedQuery?.type && isValidCustomLogoType(parsedQuery.type) ? parsedQuery.type : "logo";
  const logoDefinition = customLogoDefinitions[type];
