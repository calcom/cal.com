...
const mockFindByPhoneNumberWithUserAndTeam = vi.fn();
...
vi.mock("@calcom/lib/server/repository/PrismaPhoneNumberRepository", () => ({
  PrismaPhoneNumberRepository: vi.fn().mockImplementation(() => ({
    findByPhoneNumberWithUserAndTeam: mockFindByPhoneNumberWithUserAndTeam,
  })),
}));
...
describe("Retell AI Webhook Handler", () => {
  ...
  it("should process call_analyzed event with valid phone number and sufficient credits", async () => {
    ...
    mockFindByPhoneNumberWithUserAndTeam.mockResolvedValue(mockPhoneNumber);
  ...
  it("should handle team phone numbers", async () => {
    ...
    mockFindByPhoneNumberWithUserAndTeam.mockResolvedValue(mockTeamPhoneNumber);
  ...
  it("should handle missing total_duration_seconds", async () => {
    ...
    expect(response.status).toBe(200);
    expect(mockFindByPhoneNumberWithUserAndTeam).not.toHaveBeenCalled();
  });

  it("should handle phone number not found", async () => {
    vi.mocked(Retell.verify).mockReturnValue(true);
    mockFindByPhoneNumberWithUserAndTeam.mockResolvedValue(null);
