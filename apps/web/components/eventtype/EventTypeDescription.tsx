      ...
      <div className={classNames("text-neutral-500 dark:text-white", className)}>
        ...
        <ul className="mt-2 flex space-x-4 rtl:space-x-reverse ">
          <li className="flex whitespace-nowrap">
            <ClockIcon className="text-neutral-400 mt-0.5 mr-1.5 inline h-4 w-4" aria-hidden="true" />
            {eventType.length}m
          </li>
          {eventType.schedulingType ? (
            <li className="flex whitespace-nowrap">
              <UsersIcon className="text-neutral-400 mt-0.5 mr-1.5 inline h-4 w-4" aria-hidden="true" />
              {eventType.schedulingType === SchedulingType.ROUND_ROBIN && t("round_robin")}
              {eventType.schedulingType === SchedulingType.COLLECTIVE && t("collective")}
            </li>
          ) : (
            <li className="flex whitespace-nowrap">
              <UserIcon className="text-neutral-400 mt-0.5 mr-1.5 inline h-4 w-4" aria-hidden="true" />
              {t("1_on_1")}
            </li>
          )}
          {eventType.price > 0 && (
            <li className="flex whitespace-nowrap">
              <CreditCardIcon className="text-neutral-400 mt-0.5 mr-1.5 inline h-4 w-4" aria-hidden="true" />
