...
export const CompletePaymentDialog = (props: IRescheduleDialog) => {
  const { t } = useLocale();
  const utils = trpc.useContext();
  const { isOpenDialog, setIsOpenDialog, bookingId } = props;
  const [chargeError, setChargeError] = useState(false);

  const completePaymentMutation = trpc.viewer.payments.completePayment.useMutation({
    onSuccess: () => {
    ...
    onError: () => {
      showToast("Couldn't charge card", "error");
      setChargeError(true);
    },
  });
          ...
          <div className="pt-1">
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={completePaymentMutation.isLoading || chargeError}
