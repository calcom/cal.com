# Database Migrations
As described in the [upgrade guide](https://docs.cal.com/self-hosting/upgrade), you should use the `yarn workspace @calcom/web prisma migrate dev` or `yarn workspace @calcom/web prisma migrate deploy` command to update the database.
...
```sh
yarn workspace @calcom/web prisma migrate resolve --applied migration_name
```
