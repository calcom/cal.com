...
3. Create a SAML application with your Identity Provider (IdP) using the instructions in the [SAML Setup](#saml-registration-with-identity-providers) section below.
4. You will need the XML metadata from your IdP later, so keep it accessible.
5. Remember to configure access to the IdP SAML app for all your users (who need access to Cal).
6. Log in to one of the admin accounts configured in `SAML_ADMINS` and then navigate to Settings -> Security.
7. You should see a SAML configuration section, copy and paste the XML metadata and click on Save.
...
- **Assertion consumer service URL / Single Sign-On URL / Destination URL:** `https://<Cal.com URL>/api/auth/saml/callback` (Replace the placeholder with the URL for your self-hosted Cal instance)
...
5. Visit `{BASE_URL}/settings/security/sso` and you should see the SSO configuration page.
6. Click on Configure SSO with OIDC, and then enter the Client Secret, Client ID and Well known URL from Step 3, and click save.
