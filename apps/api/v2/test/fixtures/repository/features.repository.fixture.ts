...
export class FeaturesRepositoryFixture {
  ...
  constructor(module: TestingModule) {
    ...
    this.teamFeatureRepository = new PrismaTeamFeatureRepository(this.prismaWriteClient);
  }
  ...
  async setTeamFeatureState(
    ...
    if (input.state === "inherit") {
      await this.teamFeatureRepository.delete(input.teamId, input.featureId as FeatureId);
    } else {
      await this.teamFeatureRepository.upsert(
        input.teamId,
        input.featureId as FeatureId,
        input.state === "enabled",
        input.assignedBy ?? "test"
      );
    }
  }

  async disableFeatureForTeam(teamId: number, featureSlug: string) {
    await this.teamFeatureRepository.delete(teamId, featureSlug as FeatureId);
  }
