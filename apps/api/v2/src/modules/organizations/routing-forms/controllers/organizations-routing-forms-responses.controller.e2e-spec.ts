...
describe("OrganizationsRoutingFormsResponsesController", () => {
  ...
  describe(`POST /v2/organizations/:orgId/routing-forms/:routingFormId/responses`, () => {
    ...
    it("should return 403 when routing form does not exist for the user", async () => {
      return request(app.getHttpServer())
        .post(
        ...
        .then((response) => {
          expect(response.body.error.message).toContain(
            `IsUserRoutingForm - Routing Form with id=non-existent-id is not owned by user with id=${user.id}.`
          );
        });
    });

    it("should return 403 when routing form belongs to different user", async () => {
      ...
      expect(response.body.error.message).toContain(
        `IsUserRoutingForm - Routing Form with id=${otherUserRoutingForm.id} is not owned by user with id=${user.id}.`
      );
