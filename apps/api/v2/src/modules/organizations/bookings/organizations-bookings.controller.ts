...
export class OrganizationsBookingsController {
  ...
  @Get("/statistics")
  @ApiOperation({ summary: "Get organization booking statistics" })
  @Roles("ORG_ADMIN")
  @PlatformPlan("ESSENTIALS")
  @HttpCode(HttpStatus.OK)
  async getOrganizationBookingStatistics(
    @Query() queryParams: GetOrganizationsBookingsStatisticsInput,
    @Param("orgId", ParseIntPipe) orgId: number,
    @GetUser() user: UserWithProfile
  ): Promise<GetOrganizationsBookingsStatisticsOutput_2024_08_13> {
    const { userIds, ...restParams } = queryParams;

    const statistics = await this.bookingsService.getBookingsStatistics(
      { ...restParams },
      { email: user.email, id: user.id, orgId },
      userIds
    );
