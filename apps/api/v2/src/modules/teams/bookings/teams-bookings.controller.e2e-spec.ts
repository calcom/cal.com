...
describe("Teams Bookings Endpoints 2024-08-13", () => {
  describe("Standalone team bookings", () => {
    ...
    describe("filters", () => {
      ...
      it("should filter by eventTypeIds", async () => {
        return request(app.getHttpServer())
      ...
      it("should filter by teamMemberIds", async () => {
        return request(app.getHttpServer())
          ...
          .then(async (response) => {
            ...
            expect(responseBody.data).toBeDefined();
            const data: BookingData[] = responseBody.data;
            // Verify all returned bookings have the team admin as a host
            data.forEach((booking) => {
              if ("hosts" in booking && Array.isArray(booking.hosts)) {
      ...
      it("should filter by teamMemberEmails", async () => {
        return request(app.getHttpServer())
      ...
      it("should filter by afterStart date", async () => {
        const afterStartDate = new Date(Date.UTC(2030, 0, 10, 12, 0, 0)).toISOString();
        return request(app.getHttpServer())
          .get(`/v2/teams/${standaloneTeam.id}/bookings?afterStart=${afterStartDate}`)
          .set(CAL_API_VERSION_HEADER, VERSION_2024_08_13)
          .expect(200)
          .then(async (response) => {
            const responseBody: GetBookingsOutput_2024_08_13 = response.body;
            ...
            data.forEach((booking) => {
              if ("start" in booking) {
                expect(new Date(booking.start).getTime()).toBeGreaterThanOrEqual(
                  new Date(afterStartDate).getTime()
                );
              }
            });
          });
      ...
      it("should filter by beforeEnd date", async () => {
        const beforeEndDate = new Date(Date.UTC(2030, 0, 11, 12, 0, 0)).toISOString();
        return request(app.getHttpServer())
          .get(`/v2/teams/${standaloneTeam.id}/bookings?beforeEnd=${beforeEndDate}`)
          .set(CAL_API_VERSION_HEADER, VERSION_2024_08_13)
          .expect(200)
          .then(async (response) => {
            const responseBody: GetBookingsOutput_2024_08_13 = response.body;
            ...
            data.forEach((booking) => {
              if ("end" in booking) {
                expect(new Date(booking.end).getTime()).toBeLessThanOrEqual(
                  new Date(beforeEndDate).getTime()
                );
              }
            });
          });
