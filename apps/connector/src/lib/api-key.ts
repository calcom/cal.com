...
export function isApiKey(token: string, prefix = "calid_"): boolean {
  return token.startsWith(prefix) && token.length > prefix.length;
}

export function stripApiKey(apiKey: string, prefix = "calid_"): string {
...
export function generateApiKey(prefix = "calid_"): { apiKey: string; hashedKey: string } {
