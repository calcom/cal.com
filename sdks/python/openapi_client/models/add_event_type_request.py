...
class AddEventTypeRequest(BaseModel):
    """
    AddEventTypeRequest
    """ # noqa: E501
    length: StrictInt = Field(description="Duration of the event type in minutes")
    metadata: Dict[str, Any] = Field(description="Metadata relating to event type. Pass {} if empty")
    title: StrictStr = Field(description="Title of the event type")
    slug: StrictStr = Field(description="Unique slug for the event type")
    ...
    parent_id: Optional[StrictInt] = Field(default=None, description="EventTypeId of the parent managed event", alias="parentId")
    ...
    __properties: ClassVar[List[str]] = ["length", "metadata", "title", "slug", "hosts", "hidden", "scheduleId", "position", "teamId", "periodType", "periodStartDate", "periodEndDate", "periodDays", "periodCountCalendarDays", "requiresConfirmation", "recurringEvent", "disableGuests", "hideCalendarNotes", "minimumBookingNotice", "beforeEventBuffer", "afterEventBuffer", "schedulingType", "price", "parentId", "currency", "slotInterval", "successRedirectUrl", "description", "locations"]
    ...
    def scheduling_type_validate_enum(cls, value):
        ...
        if value not in ('ROUND_ROBIN', 'COLLECTIVE', 'MANAGED'):
