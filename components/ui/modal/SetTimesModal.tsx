...
export default function SetTimesModal(props: SetTimesModalProps) {
  ...
  const isValidTime = (startTime: number, endTime: number) => {
    ...
    if (new Date(startTime) > new Date(endTime)) {
      message = t("error_end_time_before_start_time");
      isValid = false;
    }
    if (endTime > 1440) {
      message = t("error_end_time_next_day");
      endHoursRef.current.value = Math.floor(props.endTime / 60) as unknown as string;
      endMinsRef.current.value = Math.floor(props.endTime % 60) as unknown as string;
      isValid = false;
    }
    !isValid && showToast(message, "error");
    return isValid;
  };
  ...
  return (
    <div
      ...
      <div className="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:w-full sm:max-w-lg sm:p-6 sm:align-middle">
          ...
          <div className="flex mb-4">
            ...
            <div>
              ...
              <input
                ...
                name="hours"
                id="startHours"
                className="block w-full border-gray-300 rounded-md shadow-sm focus:border-brand focus:ring-black sm:text-sm"
                placeholder="9"
