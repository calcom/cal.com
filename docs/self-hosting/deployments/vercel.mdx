...
<Warning>
  Sometimes, yarn install might fail during deployment on Vercel, in which case, you can use
  ```YARN_ENABLE_IMMUTABLE_INSTALLS=false yarn install``` as the install command instead.
</Warning>

#### Deployment

<Steps>
  <Step title="Create a new project on Vercel"></Step>
  <Step title="Import from your forked repository"></Step>
  <Step title="Set the Environment Variables"></Step>
  <Step title="Set the root directory to `apps/web`"></Step>
  <Step title="Override the build command">
    Override to: ``` cd ../.. && yarn build --include-dependencies --no-deps ```
  </Step>
  <Step title="Hit Deploy"></Step>
</Steps>
