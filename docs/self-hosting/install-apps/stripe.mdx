...
4. Add `<Cal.com URL>/api/integrations/stripepayment/callback` as redirect URL, replacing Cal.com URL with the URI at which your application runs

5. Copy your client*id (`ca*...`) to `STRIPE_CLIENT_ID` in the .env file.

6. Open [Stripe Webhooks](https://dashboard.stripe.com/webhooks) and add `<Cal.com URL>/api/integrations/stripepayment/webhook` as webhook for connected applications.
