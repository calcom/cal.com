...
1. Set in the .env file
    ...
    ```
2. Set DATABASE_URL in the .env file equal to the prisma connection string
    ```
    DATABASE_URL="prisma://****:****@****"
    ```
...
For a production build, **please make sure** to set up [E2E testing](/developing-with-cal.com/local-development#e2e-testing) and [Upgrading](/self-hosting/upgrading) the database from earlier version, and the proceed to build as follows:
...
<Card href="/self-hosting/deployments" title="Deployments" icon="rocket">
    For more details on specific deployments 
</Card>