...
| availabilityLoaded   | When all availability/booking slots have finished loading for a booking page.                        | `slots: object` // Available time slots organized by date <br /> `eventTypeId: number` // ID of the event type <br /> `startDate: string` // Start date of the loaded period <br /> `endDate: string` // End date of the loaded period <br /> `timezone: string` // Timezone for the availability data |
