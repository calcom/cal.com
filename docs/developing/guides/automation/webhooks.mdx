...
<iframe
  ...
  height="315"
  src="https://www.youtube.com/embed/Yy9me5E09LA?si=_2Mz-CKL2ajsFivd"
...
<Note>
  Please note that the webhooks can be associated with user as well as individual event types, including team
  event types.
</Note>
...
<Steps>
  ...
  <Step title="Event triggers">
    ...
    `Meeting Ended` - `Instant Meeting Created`
  </Step>
  <Step title="Secret">
...
<Steps>
  ...
  <Step title="Create an HMAC using the secret key">
    Use the **secret key** to create an HMAC, and update it with the webhook payload received to generate an
    SHA256 hash.
  </Step>
  <Step title="Verify the payload authenticity">
    Compare the hash received in the header of the webhook `(X-Cal-Signature-256)` with the one you created
    using the **secret key** and the body of the payload. If they don't match, the received payload has been
    adulterated and cannot be trusted.
  </Step>
</Steps>
