...
### Booking Brief (AI-powered host briefings)

Booking Brief is an optional feature that automatically generates an AI-powered briefing for the host 30 minutes before each meeting. It uses the attendee name, email, event type, and form responses already captured at booking time to produce an attendee summary, suggested agenda, opening question, and prep tip.

To enable it, add your Anthropic API key to your `.env` file:

```bash
ANTHROPIC_API_KEY=sk-ant-your-key-here
```

Get a key at https://console.anthropic.com/settings/keys. Each briefing costs under $0.01 in API credits. If the variable is not set, the feature is completely inactive and has no effect on existing behavior.

For local development, you can set the following variable to receive the briefing email immediately after booking instead of waiting 30 minutes:

```bash
BOOKING_BRIEF_SEND_IMMEDIATELY=true
```

Leave `BOOKING_BRIEF_SEND_IMMEDIATELY` unset in production.
