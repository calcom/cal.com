...
When creating or updating event types via the Platform API, you can now include the `conditionalOn` property:

```typescript
import { api } from "@calcom/platform-libraries";

const eventType = await api.eventTypes.create({
  // ... other event type properties
  bookingFields: [
    {
      type: "radio",
      slug: "how-did-you-hear",
