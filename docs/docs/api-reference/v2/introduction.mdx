...
**Important for Platform customers:** Managed user access tokens ensure that actions are performed on behalf of specific managed users while maintaining proper OAuth client attribution. All data created using these tokens is still associated with your OAuth client.
...
**Example request with managed user access token:**
```javascript
...
**Token management:**
- **Validity period:** Access tokens are valid for 60 minutes and refresh tokens for 1 year
- **Refreshing tokens:** Use the refresh endpoint [API reference](https://cal.com/docs/api-reference/v2/oauth/post-v2oauth-refresh). After refreshing you will receive new access and refresh tokens that you must store in your database
- **Recovering tokens:** If you lose managed user's access or refresh tokens, you can force refresh them using OAuth client credentials [API reference](https://cal.com/docs/api-reference/v2/platform-managed-users/force-refresh-tokens)
