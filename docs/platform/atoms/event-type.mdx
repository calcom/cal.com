...
| Method | Description |
|:-------|:------------|
| validateForm | Validates the current form state and returns a promise with validation results |
| handleFormSubmit | Programmatically submits the form, triggering the same validation and submission flow as clicking the save button |

The `validateForm` method returns a `FormValidationResult` object with:
