...
| apps                | No       | Array of conferencing app slugs to display in the dropdown. If provided, only these apps will be shown (if not already installed). Valid values are `'google_meet'`, `'zoom'`, and `'office365_video'`.    |
| disableBulkUpdateEventTypes | No | Boolean value to disable the bulk update of event types when changing conferencing app settings. Default is `false`. |
