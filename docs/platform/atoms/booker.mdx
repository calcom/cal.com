...
<Note>
  **Requirements for Dynamic Bookings:** All specified users must have `allowDynamicBooking` enabled in their
  settings (this is enabled by default, so only check this setting if dynamic bookings aren't working). The
  combined availability is automatically resolved.
</Note>
