...
Cal Provider is used to setup scheduling within your app. It is used in the root of your app, be it \_app.js or \_app.tsx in case of
...
| isEmbed            | No       | Only set to `true` when you render the managed `<BookerEmbed />` component. It flags outbound API requests as Cal-hosted embed traffic, which bypasses OAuth client redirects/emails. Keep it `false` for the regular Booker so that your OAuth client settings remain in effect.                                                                                                                                                                   |

> ⚠️ Setting `isEmbed={true}` while using the standard Booker will make the API treat your calls as Cal-managed embed requests. In that mode the backend ignores the OAuth client's redirect, email, and calendar settings and uses the embed defaults instead.
