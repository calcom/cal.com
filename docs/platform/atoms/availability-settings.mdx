...
import { AvailabilitySettings } from "@calcom/atoms";
import type { AvailabilitySettingsPlatformWrapperRef } from "@calcom/atoms";

export function AvailabilityWithValidation() {
  const availabilityRef = useRef<AvailabilitySettingsPlatformWrapperRef>(null);
...
| handleFormSubmit | Programmatically submits the availability form, triggering the same validation and submission flow as clicking the save button |
