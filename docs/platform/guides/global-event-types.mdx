...
Global/team event types with `assignAllTeamMembers: true` act as a template that automatically creates a per‑user child event type for each managed user. The team/global booking link is shared and cannot be used to force‑select a particular user’s availability.  
If you need to book a specific managed user with their own availability, use that user’s child event type link (e.g., `/{username}/{eventSlug}`) or programmatically create per‑user event types via the API.
</Warning>
