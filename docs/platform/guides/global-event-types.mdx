...
Global event types (team event types with `assignAllTeamMembers: true`) are designed to create a *shared* event type for all managed users in a team. **They do not allow you to generate unique booking URLs for each managed user with their own individual availability.**  
If your use case requires booking a specific managed user with their unique availability (e.g., `/username/event-type`), you must create a separate event type for each managed user using your own logic or API integration.  
Team event types are not intended for this scenario.
</Warning>

<Info>
**Platform vs Teams:**  
- **Platform**: Refers to your application integrating with Cal.com via the API and managed users.
- **Teams**: Are Cal.com entities that group users and allow for shared/team event types.
Team event types are for collective/round robin/managed scheduling within a team, not for creating unique per-user booking links.
</Info>
