...
2. Add an OAuth client.
    ...
    3. Booking, reschedule and cancel URLs: URLs of pages where users land after a successful booking or if they want to reschedule or cancel a booking. We will pass information in URL when redirecting to your pages and you will use our hooks and components to implement the pages. See [this guide](/docs/platform/booking-redirects).
...
Second, if `timeZone` is passed in the request, then user's default schedule from 9AM to 5PM will be created in the specified timezone. Then, using `AvailabilitySettings` atom, user can customize their availability. However,
