...
Each refresh token is valid for 1 year. If your customers are active then when access token is refreshed the refresh token is refreshed too, so all good there.
