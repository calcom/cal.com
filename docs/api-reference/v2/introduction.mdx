...
If you are a platform customer, then you don't need an API key and must instead use OAuth credentials or managed user access token. We cover when to use which below.
...
3. Refreshing tokens of managed user [API reference](https://cal.com/docs/api-reference/v2/oauth/post-v2oauth-refresh)
