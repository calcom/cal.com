...
We provide an official Postman collection for quick API testing and integration.

<Card
  title="Download Postman Collection"
  icon="download"
  href="https://raw.githubusercontent.com/calcom/cal.com/main/docs/api-reference/v2/postman-collection.json"
>
  Import directly into Postman to start testing the Cal.com API
</Card>

### How to use

1. **Download** the collection using the link above or import directly in Postman using the URL
2. **Import** into Postman (File → Import → Link/File)
3. **Configure variables** - Set the following collection variables:
   - `bearerToken`: Your Cal.com API key (for API key authentication)
   - `clientId`: Your OAuth client ID (for Platform OAuth authentication)
   - `clientSecret`: Your OAuth client secret (for Platform OAuth authentication)
4. **Start testing** - Select any endpoint and send requests!

The collection is automatically generated from our OpenAPI specification and stays in sync with the latest API changes.
