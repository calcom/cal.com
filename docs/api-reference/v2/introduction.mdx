...
if invalid credential is provided, the rate limit is also 10 requests per minute. So always provide a valid credential for all requests to the api to have a proper rate limit!