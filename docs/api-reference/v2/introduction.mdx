...
## Rate limits
1. API keys have 120 requests per minute by default. If you need more requests per API key, please contact support at https://app.cal.com/support.
2. OAuth clients have 500 requests per minute.
3. Access tokens have 500 requests per minute.

Importantly, you have to use a credential for all endpoints and requests to API v2 to have the rate limit above.
If a request is made to a public endpoint without a credential (API key, OAuth client credentials or access token), the rate limit is 10 requests per minute. Also,
if invalid credentials are provided, the rate limit is also 10 requests per minute. So always provide valid credentials for all requests to the API to have a proper rate limit.