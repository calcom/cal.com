...
**V2 request body examples:**

Update to physical address:
```json
{
  "location": {
    "type": "address",
...
**Key differences:**
- This is a new endpoint in v2 (not available in v1)
- Requires `cal-api-version: 2024-08-13` header (returns 404 without it)
- Requires BOOKING_WRITE permission
- **Important limitation:** Updates the location in Cal.com but does NOT update the corresponding external calendar event. The old location will persist in the calendar event.
