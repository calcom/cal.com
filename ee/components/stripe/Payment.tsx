...
const CARD_OPTIONS = {
  ...
  classes: {
    base: "block p-2 w-full border-solid border-2 border-gray-300 rounded-md shadow-sm    focus-within:ring-black focus-within:border-black sm:text-sm",
  },
  style: {
    base: {
      color: "#000",
...
export default function PaymentComponent(props: Props) {
  ...
  return (
    <form id="payment-form" className="mt-4" onSubmit={handleSubmit}>
      ...
      {state.status === "error" && (
        <div className="mt-4 text-gray-700  text-center" role="alert">
          {state.error.message}
        </div>
      )}
    </form>
