...
ensureWebServerIsReady(webServer[0]);
...
function ensureWebServerIsReady(webServer: { port?: number; url?: string }) {
  delete webServer.port;
  webServer.url = process.env.NEXT_PUBLIC_WEBAPP_URL;
  console.log("Ensuring that web is 200 before starting tests");
}
