...
**Proper sequence:**
1. Commit locally
2. Run local checks scoped to changes — type-check the affected workspace (e.g. `yarn workspace @calcom/web type-check`) and `yarn biome check --write --changed --since=origin/main`
3. Push to remote
4. Monitor CI status
