...
export const intercom = {
    ...
    async deleteContact(email: string): Promise<IntercomResponse<boolean>> {
        try {
            const contactResponse = await this.getContactByEmail(email);

            if (contactResponse.error) {
                return { error: contactResponse.error };
            }

            const contact = contactResponse.data;
            if (!contact) {
                return { data: true }; // Already gone or never existed
            }

            const res = await fetch(`${INTERCOM_API_ENDPOINT}/contacts/${contact.id}`, {
                method: "DELETE",
                headers: {
                    Authorization: `Bearer ${INTERCOM_API_TOKEN}`,
                    "Content-Type": "application/json",
            ...
            console.error(`Unexpected error while deleting contact: ${safeStringify(err)}`);
            return {
                error: "Unexpected error while deleting contact",
            };
        }
    },
};
