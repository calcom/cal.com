...
const log = logger.getSubLogger({ prefix: ["validateSmtpHost"] });
...
function isSelfHostedDangerousIP(ip: string): boolean {
  try {
    ...
    if (addr.kind() === "ipv6" && (addr as ipaddr.IPv6).isIPv4MappedAddress()) {
      return SELF_HOSTED_BLOCKED_RANGES.has((addr as ipaddr.IPv6).toIPv4Address().range());
    }
...
export async function resolveAndValidateSmtpHost(
  ...
  try {
    ...
    log.warn("DNS lookup failed for SMTP host, allowing to proceed", { host: trimmed, error });
  }
