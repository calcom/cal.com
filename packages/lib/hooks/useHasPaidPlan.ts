...
export function useHasPaidPlan() {
  ...
  const { data: hasTeamPlan, isPending: isPendingTeamQuery } = trpc.viewer.teams.queries.hasTeamPlan.useQuery();
...
export function useHasTeamPlan() {
  const { data: hasTeamPlan, isPending } = trpc.viewer.teams.queries.hasTeamPlan.useQuery();
...
export function useHasActiveTeamPlan() {
  ...
  const { data, isPending } = trpc.viewer.teams.queries.hasActiveTeamPlan.useQuery();
...
export function useHasActiveTeamPlanAsOwner() {
  ...
  const { data, isPending } = trpc.viewer.teams.queries.hasActiveTeamPlan.useQuery({ ownerOnly: true });
