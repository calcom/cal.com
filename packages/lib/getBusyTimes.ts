...
const _getBusyTimes = async (params: {
  ...
  let calendarBusyTimesWithTimeBlocksPromise;
  const timeBlocks: {
    date: Date | null;
    startTime: Date;
    endTime: Date;
  ...
  if (credentials?.length > 0 && !bypassBusyCalendarTimes) {
    calendarBusyTimesWithTimeBlocksPromise = getBusyCalendarTimes(
      credentials,
      startTime,
      endTime,
      selectedCalendars,
  ...
  if (credentials?.length > 0 && !bypassBusyCalendarTimes && calendarBusyTimesWithTimeBlocksPromise) {
    const startConnectedCalendarsGet = performance.now();
    const calendarBusyTimesWithTimeBlocks = await calendarBusyTimesWithTimeBlocksPromise;
