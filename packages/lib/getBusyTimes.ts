...
const _getBusyTimes = async (params: {
  ...
  const { getBusyTimesService } = await import("./di/containers/busy-times");
  const service = getBusyTimesService();
  return service.getBusyTimes(params);
};

export const getBusyTimes = withReporting(_getBusyTimes, "getBusyTimes");

export async function getStartEndDateforLimitCheck(
  startDate: string,
  endDate: string,
  bookingLimits?: IntervalLimit | null,
  durationLimits?: IntervalLimit | null
  ...
  const { getBusyTimesService } = await import("./di/containers/busy-times");
  const service = getBusyTimesService();
  return service.getStartEndDateforLimitCheck(startDate, endDate, bookingLimits, durationLimits);
}
...
export async function getBusyTimesForLimitChecks(params: {
  ...
  const { getBusyTimesService } = await import("./di/containers/busy-times");
  const service = getBusyTimesService();
  return service.getBusyTimesForLimitChecks(params);
}
