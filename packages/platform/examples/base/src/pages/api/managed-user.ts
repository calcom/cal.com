...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  const team = await createTeam(body.data?.user.organizationId, "Team Doe");
  await createMembership(body.data?.user.organizationId, team.id, body.data?.user.id);
  await createMembership(bodyTwo.data?.user.organizationId, team.id, bodyTwo.data?.user.id);
  await createCollectiveEventType(body.data?.user.organizationId, team.id, [
    body.data?.user.id,
