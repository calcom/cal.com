...
export const useEmbedTheme = () => {
    ...
    const [theme, setTheme] = useState(embedStore.theme || (searchParams?.get("theme") as typeof embedStore.theme));
    useEffect(() => {
...
const methods = {
    ui: function style(uiConfig: UiConfig) {
        ...
        if (stylesConfig) {
            console.warn("Cal.com Embed: `styles` prop is deprecated. Use `cssVarsPerTheme` instead to achieve the same effect. Here is a list of CSS variables that are supported. https://github.com/calcom/cal.com/blob/main/packages/config/tailwind-preset.js#L19");
        }
