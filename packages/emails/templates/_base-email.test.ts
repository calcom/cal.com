...
const {
  ...
  return {
    ...
    mockCheckIfTeamHasFeature: vi.fn().mockResolvedValue(true),
...
vi.mock("@calcom/lib/server/getServerErrorFromUnknown", () => ({
  getServerErrorFromUnknown: (err: unknown) => (err instanceof Error ? err : new Error(String(err))),
}));
...
class AttendeeScheduledEmail extends BaseEmail {
  ...
  protected async resolveSmtpConfig() {
    return mockGetConfigForOrg();
  }
...
describe("BaseEmail", () => {
  ...
  describe("org SMTP", () => {
    ...
    it("does not use org SMTP when custom-smtp-for-orgs feature is disabled", async () => {
