...
export class SalesforceGraphQLClient {
  ...
  constructor({ accessToken, instanceUrl }: { accessToken: string; instanceUrl: string }) {
    ...
    this.client = new GraphQLClient(`${instanceUrl}/services/data/${this.version}/graphql`, {
      headers: {
  ...
  async GetAccountRecordsForRRSkip(email: string): Promise<Contact[]> {
    ...
    try {
      const queryData = await this.client.request(GetAccountRecordsForRRSkip, {
        email,
        websites,
      ...
      log.error("GraphQL request failed", safeStringify({ error }));
      return [];
    }
  }
