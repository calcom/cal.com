...
export default class ZohoCRMRevertCalendarService implements Calendar {
  ...
  private createZohoCRMEvent = async (event: CalendarEvent, contacts: CalendarEvent["attendees"]) => {
    const eventPayload = {
      subject: event.title,
  ...
  async handleEventCreation(event: CalendarEvent, contacts: CalendarEvent["attendees"]) {
    ...
    return Promise.reject("Something went wrong when creating a meeting in ZOHO CRM");
  }

  async createEvent(event: CalendarEvent): Promise<NewCalendarEventType> {
    let contacts = await this.contactSearch(event);

    contacts = contacts.filter((c) => !c.error && c.results && c.results.length >= 1);
