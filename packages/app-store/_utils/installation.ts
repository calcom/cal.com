...
export async function checkInstalled(slug: string, userId: number) {
  const alreadyInstalled = await PrismaCredentialRepository.findByAppIdAndUserId({ appId: slug, userId });
  if (alreadyInstalled) {
...
export async function isAppInstalled({ appId, userId }: { appId: string; userId: number }) {
  const alreadyInstalled = await PrismaCredentialRepository.findByAppIdAndUserId({ appId, userId });
