...
const VIDEO_APPS_MAP = Object.keys(videoAppsMetaData).reduce((store, key) => {
  const metadata = videoAppsMetaData[key as keyof typeof videoAppsMetaData] as AppMeta;
...
const ALL_VIDEO_APPS = Object.values(VIDEO_APPS_MAP);

export const getVideoApps = (credentials: CredentialDataWithTeamName[], filterOnCredentials?: boolean) => {
  const apps = ALL_VIDEO_APPS.reduce((reducedArray, appMeta) => {
