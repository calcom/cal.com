...
import type { TCalidFormsInputSchema } from "./forms.schema";
...
interface CalidFormsHandlerOptions {
  ...
  input: TCalidFormsInputSchema;
}
const log = logger.getSubLogger({ prefix: ["[calidFormsHandler]"] });

export const calidFormsHandler = async ({ ctx, input }: CalidFormsHandlerOptions) => {
  const { prisma, user } = ctx;

  const where = getPrismaWhereFromFilters(user, input?.filters);
  log.debug("Getting calid forms where", JSON.stringify(where));
  ...
  async function buildFormsWithReadOnlyStatus() {
    ...
    const formsWithReadonlyStatus = settledFormsWithReadonlyStatus.map((result, index) => {
      ...
      log.error(`Error getting calid form ${form.id}: ${safeStringify(result.reason)}`);
...
export default calidFormsHandler;
type SupportedFilters =
  | Omit<NonNullable<NonNullable<TCalidFormsInputSchema>["filters"]>, "upIds">
  | undefined;
