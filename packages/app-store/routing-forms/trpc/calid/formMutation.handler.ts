...
import {
  canEditCalIdEntity,
  calidEntityPrismaWhereClause,
} from "@calcom/lib/calIdEntityPermissionUtils.server";
...
import type { TCalIdFormMutationInputSchema } from "./formMutation.schema";

interface calIdFormMutationHandlerOptions {
  ctx: {
    prisma: PrismaClient;
    user: NonNullable<TrpcSessionUser>;
  };
  input: TCalIdFormMutationInputSchema;
}

export const calIdFormMutationHandler = async ({ ctx, input }: calIdFormMutationHandlerOptions) => {
  const { user, prisma } = ctx;
  const { name, id, description, disabled, addFallback, duplicateFrom, shouldConnect } = input;
  let teamId = input.calIdTeamId;
  const settings = input.settings;
  if (!(await isCalIdFormCreateEditAllowed({ userId: user.id, formId: id, targetCalIdTeamId: teamId }))) {
    throw new TRPCError({
      code: "FORBIDDEN",
    });
  }
  ...
  const dbSerializedForm = dbForm
    ? await getCalIdSerializableForm({ form: dbForm, withDeletedFields: true })
    : null;
  ...
  if (teamId && settings?.sendUpdatesTo?.length) {
    const sendUpdatesTo = await prisma.calIdMembership.findMany({
      where: {
        calIdTeamId: teamId,
        userId: {
  ...
  async function getRoutesAndFieldsForDuplication({
    ...
    const sourceForm = await prisma.app_RoutingForms_Form.findFirst({
      where: {
        ...calidEntityPrismaWhereClause({ userId }),
        id: duplicateFrom,
      },
      select: {
        id: true,
    ...
    if (
      !(await canEditCalIdEntity({ userId: sourceForm.userId, calIdTeamId: sourceForm.calIdTeamId }, userId))
    ) {
      throw new TRPCError({
        code: "NOT_FOUND",
    ...
    return { teamId: sourceForm.calIdTeamId, routes, fields };
  }
...
export default calIdFormMutationHandler;
