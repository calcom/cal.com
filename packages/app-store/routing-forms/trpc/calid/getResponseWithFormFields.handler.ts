...
async function getResponseWithFormFieldsHandler({ ctx, input }: GetResponseWithFormFieldsOptions) {
  ...
  // TODO: To make the check stricter, we could check if the user is admin/owner of the calidTeam or a member that is the organizer.
  ...
  return {
    ...
    form: await getCalIdSerializableForm({ form: enrichCalIdFormWithMigrationData(formWithUserProfile) }),
  };
}
