...
import { Button, ConfirmationDialogContent, Dialog, DialogClose, DialogContent, DialogFooter, Dropdown, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, Form, showToast, Switch, TextAreaField, TextField, SettingsToggle, } from "@calcom/ui";
...
function NewFormDialog({ appUrl }: {
    ...
    return (<Dialog name="new-form" clearQueryParamsOnClose={["target", "action"]}>
      <DialogContent className="overflow-y-auto">
        <div className="mb-1">
          <h3 className="text-emphasis !font-cal text-semibold leading-20 text-xl font-medium" id="modal-title">
...
export const FormActionsDropdown = ({ children, disabled, }: {
    disabled?: boolean;
...
export const FormAction = forwardRef(function FormAction<T extends typeof Button>(props: FormActionProps<T>, forwardedRef: React.ForwardedRef<HTMLAnchorElement | HTMLButtonElement>) {
    ...
    const actionData: Record<FormActionType, ButtonProps & {
        ...
        toggle: {
            ...
            <Switch disabled={!!disabled} checked={!routingForm.disabled} label={label} onCheckedChange={(checked) => toggle.onAction({ routingForm, checked })} labelOnLeading/>
          </div>);
            },
...
export const createAction = ({ router, teamId }: {
