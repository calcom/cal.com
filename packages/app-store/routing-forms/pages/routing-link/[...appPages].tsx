...
const useBrandColors = ({ brandColor, darkBrandColor, }: {
                ...
                <div className="main border-booker md:border-booker-width dark:bg-muted bg-default mx-0 rounded-md p-4 py-6 sm:-mx-4 sm:px-8 ">
                  ...
                  <form onSubmit={handleOnSubmit}>
                    <div className="mb-8">
                      ...
                      {form.description ? (<p className="min-h-10 text-subtle text-sm ltr:mr-4 rtl:ml-4">{form.description}</p>) : null}
                    </div>
                    <FormInputFields form={form} response={response} setResponse={setResponse}/>
                    <div className="mt-4 flex justify-end space-x-2 rtl:space-x-reverse">
                      <Button className="dark:bg-darkmodebrand dark:text-darkmodebrandcontrast dark:hover:border-darkmodebrandcontrast dark:border-transparent" loading={responseMutation.isLoading} type="submit" color="primary">
                        {t("submit")}
                      </Button>
                    </div>
                  </form>
...
function getUrlSearchParamsToForward(response: Response, fields: NonNullable<Props["form"]["fields"]>) {
    ...
    Object.entries(response).forEach(([key, fieldResponse]) => {
        ...
        const valueAsStringOrStringArray = typeof fieldResponse.value === "number" ? String(fieldResponse.value) : fieldResponse.value;
...
export const getServerSideProps = async function getServerSideProps(context: AppGetServerSidePropsContext, prisma: AppPrisma) {
