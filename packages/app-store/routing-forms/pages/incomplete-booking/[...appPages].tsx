...
function Page({ form }: { form: RoutingFormWithResponseCount }) {
  const { t } = useLocale();
  const { data, isLoading } = trpc.viewer.quarantine.appRoutingForms.getIncompleteBookingSettings.useQuery({
    formId: form.id,
  });

  const mutation = trpc.viewer.quarantine.appRoutingForms.saveIncompleteBookingSettings.useMutation({
    onSuccess: () => {
