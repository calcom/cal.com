...
import { calidEntityPrismaWhereClause } from "@calcom/lib/calIdEntityPermissionUtils.server";
...
const log = logger.getSubLogger({ prefix: ["getCalIdSerializableForm"] });
/**
 * Doesn't have deleted fields by default
 */
export async function getCalIdSerializableForm<TForm extends App_RoutingForms_Form>({
  form,
    ...
    for (const [, route] of Object.entries(parsedRoutes)) {
      if (isRouter(route)) {
        const router = await prisma.app_RoutingForms_Form.findFirst({
          where: {
            id: route.id,
            ...calidEntityPrismaWhereClause({ userId: userId }),
          },
        });
        if (!router) {
        ...
        const parsedRouter = await getCalIdSerializableForm({ form: router });
