---
items:
  - /api/app-store/rainbow/1.jpg
  - /api/app-store/rainbow/2.jpg
  - /api/app-store/rainbow/3.jpg
---

<Slider items={items} />

Token gate bookings based on NFTs, DAO tokens, and ERC-20 tokens.

Token gate your bookings. Rainbow supports dozens of trusted Ethereum wallet apps to verify token ownership.

<strong>Available blockchains are Ethereum mainnet, Arbitrum, Optimism, and Polygon mainnet.</strong>

