...
export default class SalesforceAppService extends RevertCRMAppService {
  ...
  protected contactSearch = async (event: CalendarEvent) => {
    const result = event.attendees.map(async (attendee) => {
      ...
      const bodyRaw = JSON.stringify({
        searchCriteria: `FIND {${attendee.email}} IN EMAIL FIELDS RETURNING Contact(Id, FirstName, LastName, Email)`,
      });
  ...
  async createEvent(event: CalendarEvent): Promise<NewCalendarEventType> {
    ...
    if (contacts && contacts.length) {
      if (contacts.length === event.attendees.length) {
        ...
        return Promise.reject({
          calError: "Something went wrong when creating non-existing attendees in SFDC",
        });
      }
    } else {
    ...
    return Promise.reject({
      calError: "Something went wrong when searching/creating the attendees in SFDC",
    });
  }
