...
export function assertSafeResolvedIp(ip: string): void {
  ...
  // ── Cloud metadata IPs that are NOT caught by private-range checks ────────
  // These IPs are publicly routable (not RFC 1918 / link-local) but are used
  // exclusively as cloud platform metadata endpoints.  They must be blocked
  // explicitly here, because regex-based range checks below will NOT catch them:
  //
  //   168.63.129.16   — Azure IMDS / Azure Wire Server.  This is a public IP
  //                     assigned by Microsoft; it is reachable only from within
  //                     Azure VMs and does not fall in any RFC-1918 range.
  //
  //   100.100.100.200 — Alibaba Cloud ECS metadata endpoint.  100.64.0.0/10 is
  //                     CGNAT (RFC 6598) — shared address space that is not RFC
  //                     1918, so it passes the private-range regexes below.  In
  //                     self-hosted deployments the CGNAT block may be routable
  //                     on the internal network, making this reachable.
  //
...
export function validateExternalUrl(rawUrl: string): void {
  ...
  // Reject cloud metadata endpoints (AWS, GCP, Azure, Alibaba Cloud)
  const blockedHostnames = [
    "169.254.169.254", // AWS/GCP/Azure IMDS (link-local, also caught by IP range check)
    "metadata.google.internal", // GCP
    "168.63.129.16", // Azure IMDS (public IP, NOT caught by private-range checks)
    "100.100.100.200", // Alibaba Cloud ECS metadata (CGNAT range, may bypass range checks in self-hosted mode)
  ];
