...
describe("validateExternalUrl", () => {
  ...
  it("rejects cloud metadata endpoint by hostname (GCP)", () => {
    expect(() => validateExternalUrl("https://metadata.google.internal/")).toThrow(
      /metadata|not allowed/i
    );
  });

  it("rejects Azure IMDS endpoint (168.63.129.16 — public IP, not caught by range checks)", () => {
    expect(() => validateExternalUrl("https://168.63.129.16/")).toThrow(/metadata|not allowed/i);
  });

  it("rejects Alibaba Cloud ECS metadata endpoint (100.100.100.200 — CGNAT range)", () => {
    expect(() => validateExternalUrl("https://100.100.100.200/latest/meta-data/")).toThrow(
      /metadata|not allowed/i
    );
  });
...
describe("assertSafeResolvedIp", () => {
  ...
  // ── Cloud metadata IPs — public but explicitly blocked ────────────────────
  // These IPs are NOT in any RFC-1918 / link-local range, so the regex checks
  // below would pass them.  assertSafeResolvedIp must block them explicitly so
  // that a hostname which DNS-resolves to one of these IPs is rejected at the
  // post-DNS validation step (the layer cubic flagged in round-17).

  it("rejects 168.63.129.16 (Azure IMDS — public IP, not in RFC-1918 range)", () => {
    // Azure Wire Server / IMDS endpoint: publicly routed but only reachable
    // from within Azure VMs.  Must be blocked post-DNS to prevent SSRF via a
    // hostname that resolves to this address.
    expect(() => assertSafeResolvedIp("168.63.129.16")).toThrow(/metadata|not allowed/i);
  });
