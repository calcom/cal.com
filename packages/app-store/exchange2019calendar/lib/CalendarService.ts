...
export default class ExchangeCalendarService implements Calendar {
  ...
  constructor(credential: CredentialPayload) {
    ...
    this.integrationName = "exchange2019_calendar";
    ...
    this.exchangeVersion = ExchangeVersion.Exchange2019;
  }
  ...
  private getExchangeService(): ExchangeService {
    try {
      ...
      // Set options for better compatibility with Exchange 2019
      exch1.SetXHROptions({
      ...
      this.log.error(`Error initializing Exchange service: ${error}`);
      throw new Error(`Failed to initialize Exchange 2019 service: ${error.message}`);
    }
  }
}
