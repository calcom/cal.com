import useApp from "@calcom/lib/hooks/useApp";
import { Button } from "@calcom/ui";

import useMeQuery from "@lib/hooks/useMeQuery";

export default function GreetmateButton() {
  const { data: user } = useMeQuery();
  const { data: greeterApp } = useApp("greetmate-ai");

  // Make sure that greeterApp is installed. We shouldn't show the button when app is not installed
  if (!user || !greeterApp) {
    return null;
  }

  const goToGreetmate = () => {
    window.open("https://www.greetmate.ai/login");
  };

  return (
    <Button onClick={goToGreetmate} className="cursor-pointer justify-center text-center">
      <span class="flex items-center gap-2">
        <svg
          height={16}
          width={16}
          id="Layer_2"
          data-name="Layer 2"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 166.29 184.32">
          <g id="Layer_1-2" data-name="Layer 1">
            <g>
              <g>
                <g>
                  <path fill="#eef2fc" d="M24.43,24.32l-.19.18-.14-.14c.11-.02.21-.03.34-.03Z" />
                  <path
                    fill="#eef2fc"
                    d="M104.75,143.11c-6.89,3.51-13.95,4.23-18.67,4.23s-9.36-.75-12.74-2.08c-3.36-1.33-4.95-2.9-5.34-3.94-.32-.9.27-2.19,1.73-4.76,1.92-3.43,4.55-8.09,2.77-13.97-3.49-11.28-18.7-14.55-32.7-16.71l-.19-.03-.19-.03c-6.76-1.03-13.68-2.07-19.93-2.07-6.94,0-12.13,1.39-15.77,4.28-.05-.22-.11-.43-.16-.66-.03-.11-.05-.21-.08-.32-.22-.95-.45-1.89-.66-2.82-.26-1.14-.48-2.26-.7-3.36-.06-.34-.13-.67-.19-.99-.18-.87-.34-1.73-.48-2.58-.13-.8-.27-1.6-.38-2.39-.1-.64-.19-1.3-.27-1.92-.1-.69-.18-1.35-.26-2.02-.1-.85-.19-1.7-.26-2.53-.11-1.23-.19-2.42-.22-3.59-.03-.4-.03-.79-.05-1.19-.18-7.58,1.06-13.62,4.17-16.97.21-.22.43-.43.66-.64.22-.21.46-.38.7-.56,2.47-1.76,8.27-3.75,13.39-1.65,8.51,3.49,9.72,16.28,9.72,16.39.22,2.6,2.5,4.53,5.09,4.31,2.61-.22,4.53-2.52,4.31-5.11-.08-.75-1.75-18.65-15.52-24.32-2.56-1.04-5.21-1.52-7.82-1.59-.05-.34-.16-.69-.32-.99-3.43-6.99-2.4-15.11,2.48-19.72,2.23-2.11,5.17-3.48,8.14-3.89.53-.06,1.03-.11,1.55-.13,1.38-.05,2.72.11,4.01.51,3.85,1.14,6.1,4.04,7.61,5.96,2.98,3.8,3.44,6.6,4.29,11.71.64,3.76,1.44,8.44,3.49,14.9,3.49,10.97,7.98,18.76,11.13,23.38,2.71,3.96,5.78,7.69,9.15,11.1,1.73,1.75,4.49,1.87,6.36.34.11-.1.21-.21.32-.3,1.84-1.83,1.87-4.82.05-6.66-2.98-3.03-5.7-6.33-8.09-9.81-2.8-4.07-6.79-11.02-9.93-20.91-1.84-5.8-2.55-9.95-3.17-13.62-.98-5.74-1.71-10.25-6.17-15.94-1.83-2.32-5.61-7.16-12.34-9.2-.66-.19-1.35-.37-2.02-.5,1.35-3.36,3.83-6.12,7.08-7.67,4.84-2.31,9.95-1.23,13.12.53,2.95,1.65,4.57,4.05,5.96,6.38,5.01,8.33,7.27,15.2,9.45,21.87,1.62,5.01,3.3,10.21,6.12,15.65,3.57,6.99,8.49,13.46,14.63,19.26,1.75,1.65,4.41,1.68,6.22.22.16-.14.32-.27.45-.43,1.79-1.89,1.7-4.87-.18-6.66-5.35-5.05-9.63-10.67-12.72-16.69-2.45-4.77-3.96-9.39-5.56-14.28-2.23-6.81-4.74-14.55-10.32-23.81-.93-1.55-2.58-4.33-5.42-6.86,2.02-2.63,5.51-5.98,10.67-6.94,3.6-.67,7.05-.24,9.69,1.2,2.76,1.54,4.2,5.03,9.29,18.15l.21.56c.93,2.36,1.87,4.71,2.84,7.05,1.35,3.28,2.72,6.68,3.96,10.03,4.26,11.7,11.31,23.46,21.02,34.91l-.16.16-.35.58-.08.1-.18.26c-6.14,8.83-16.42,23.58-10.21,34.72,2.96,5.29,8.06,6.57,11.79,7.5,3.17.8,4.39,1.25,4.87,2.23,2.34,4.65-6.04,18.91-18.73,25.36Z"
                  />
                </g>
                <path fill="#eef2fc" d="M24.43,24.32l-.19.18-.14-.14c.11-.02.21-.03.34-.03Z" />
                <g>
                  <path
                    fill="#eef2fc"
                    d="M26.57,28.78c-.45.13-.93.18-1.43.14-.05,0-.08-.02-.13-.02.53-.06,1.03-.11,1.55-.13Z"
                  />
                  <path fill="#eef2fc" d="M24.43,24.32l-.19.18-.14-.14c.11-.02.21-.03.34-.03Z" />
                </g>
              </g>
              <path
                fill="#eef2fc"
                d="M166.29,104.27c0,1.46-.05,2.92-.13,4.36-.91,16.52-7.39,32.14-19.24,46.45-8.2,9.92-18.84,18.06-30.79,23.55-1.68.62-3.3,1.19-4.89,1.68-8.54,2.66-17.29,4.01-26.07,4.01-2.13,0-4.26-.08-6.39-.24-18.54-1.35-34.3-7.47-46.86-18.2-7.23-6.15-14.19-14.66-19.15-23.34-.45-.77-.88-1.55-1.28-2.32-.02-.03-.03-.05-.05-.08-4.87-9.29-6.68-17.53-5.06-22.41l.02-.02c.05-.16.1-.32.16-.46.03-.11.08-.24.14-.35.13-.29.27-.56.45-.83.06-.1.11-.19.18-.27.05-.08.11-.18.18-.26l.21-.27c2.23-2.61,6.54-3.49,11.79-3.49,5.67,0,12.42,1.03,18.73,1.99l.27.05c7.8,1.2,24.05,3.7,26.34,11.12.75,2.48-.29,4.53-2.07,7.69-1.76,3.09-3.94,6.95-2.29,11.46,1.31,3.62,4.74,6.62,9.92,8.65,4.12,1.62,9.64,2.64,15.68,2.64,7.18,0,15.11-1.44,22.29-5.09,14.5-7.37,27.62-25.47,22.27-36.1-2.23-4.44-6.58-5.53-10.09-6.41-3.4-.85-5.54-1.49-6.75-3.64-3.76-6.75,5.35-19.85,9.79-26.23.14-.18.29-.37.38-.53l.02-.02c-.06,0-.13-.02-.19-.03.06.02.13.02.19.02l1.09-1.52c1.17-1.67,2.39-3.38,3.6-5.03.19-.26.37-.51.56-.75,4.81-6.52,10.99-13.86,16.68-13.86.22,0,.45,0,.67.03,10.05,1.06,19.69,25.35,19.69,48.08Z"
              />
            </g>
          </g>
        </svg>
        Go to Greetmate.ai
      </span>
    </Button>
  );
}
