...
export const getVideoAdapters = async (withCredentials: CredentialPayload[]): Promise<VideoApiAdapter[]> => {
  ...
  for (const cred of withCredentials) {
    ...
    let videoAdapterGetter = VideoApiAdapterMap[appName as keyof typeof VideoApiAdapterMap];

    // fallback: transforms zoom_video to zoom
    if (!videoAdapterGetter) {
      ...
      videoAdapterGetter = VideoApiAdapterMap[appTypeVariant as keyof typeof VideoApiAdapterMap];
    }
    ...
    const videoAdapterModule = await videoAdapterGetter();
