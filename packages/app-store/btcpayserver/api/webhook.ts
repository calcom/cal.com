...
export default async function handler(
  ...
  try {
    ...
    if (
      ...
      throw new HttpCode({
        statusCode: 401,
        message: "Missing or invalid signature format",
      });
    ...
    if (!payment)
      throw new HttpCode({
        statusCode: 404,
        message: "Cal.com: payment not found",
      });
    if (payment.success)
      return res.status(200).send({ message: "Payment already registered" });
    const key = payment.booking?.user?.credentials?.[0].key;
    if (!key)
      throw new HttpCode({
        statusCode: 404,
        message: "Cal.com: credentials not found",
      });
    ...
    if (!parsedKey.success)
      throw new HttpCode({
        statusCode: 400,
        message: "Cal.com: Invalid BTCPay credentials",
      });
    ...
    if (storeId !== data.storeId)
      throw new HttpCode({
        statusCode: 400,
        message: "Cal.com: Store ID mismatch",
      });

    const expectedSignature = signature.split("=")[1];
    const computedSignature = verifyBTCPaySignature(
      rawBody,
      expectedSignature,
      webhookSecret
    );
    ...
    const traceContext = distributedTracing.createTrace(
      "btcpayserver_webhook",
      {
