...
export const getCalendarsEvents = async (
...
export const getCalendarsEventsByKeywords = async (withCredentials: CredentialPayload[]): any => {
  const calendarCredentials = withCredentials
    .filter((credential) => credential.type.endsWith("_calendar"))
    // filter out invalid credentials - these won't work.
    .filter((credential) => !credential.invalid);
  ...
  const results = calendars.map(async (c, i) => {
    ...
    const eventBusyDates = await c.getEventsByKeywords();
    performance.mark("eventBusyDatesEnd");
    performance.measure(
      `[getAvailability for ${selectedCalendarIds.join(", ")}][$1]'`,
      "eventBusyDatesStart",
