...
thisModule.bind(ADMIN_DI_TOKENS.USER_UNBLOCK_SERVICE).toFactory((resolve: ResolveFunction) => {
...
export function loadModule(container: Parameters<ModuleLoader["loadModule"]>[0]): void {
  ...
  container.load(ADMIN_DI_TOKENS.USER_UNBLOCK_SERVICE_MODULE, thisModule);
}

export const userUnblockServiceModuleLoader: ModuleLoader = {
  token: ADMIN_DI_TOKENS.USER_UNBLOCK_SERVICE,
  loadModule,
};
