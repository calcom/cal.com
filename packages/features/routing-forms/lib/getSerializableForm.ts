...
type ZodRoutesView = typeof zodRoutesView;
type ZodFieldsView = typeof zodFieldsView;
...
export async function getSerializableForm<TForm extends App_RoutingForms_Form>({
  ...
  const fields = parsedFields as NonNullable<z.infer<ZodFieldsView>>;
  ...
  async function getEnrichedRoutesAndRouters(parsedRoutes: z.infer<typeof zodRoutes>, userId: number) {
    ...
    const routes: z.infer<ZodRoutesView> = [];
    if (!parsedRoutes) {
