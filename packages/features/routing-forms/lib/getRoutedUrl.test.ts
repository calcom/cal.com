...
describe("getRoutedUrl", () => {
  ...
  describe("CRM Fetch Attempted Flag", () => {
    it("should include cal.crmFetchAttempted=true in searchParams when fetchCrm is true", async () => {
      vi.mocked(PrismaRoutingFormRepository.findFormByIdIncludeUserTeamAndOrg).mockResolvedValue(
        mockForm as never
      );
      const redirectUrl = "test-user/30min";
      ...
      const callArgs = vi.mocked(getUrlSearchParamsToForward).mock.calls[0][0];
      const searchParams = callArgs.searchParams;
      expect(searchParams.get("cal.crmFetchAttempted")).toBe("true");
      expect(searchParams.get("cal.action")).toBe("eventTypeRedirectUrl");
    });

    it("should not include cal.crmFetchAttempted when fetchCrm is false", async () => {
      vi.mocked(PrismaRoutingFormRepository.findFormByIdIncludeUserTeamAndOrg).mockResolvedValue(
        mockForm as never
      );
      const redirectUrl = "test-user/30min";
      ...
      const context = mockContext({ email: "test@cal.com" });
      await getRoutedUrl(context, false);
      ...
      const callArgs = vi.mocked(getUrlSearchParamsToForward).mock.calls[0][0];
      const searchParams = callArgs.searchParams;
      expect(searchParams.get("cal.crmFetchAttempted")).toBeNull();
      expect(searchParams.get("cal.action")).toBe("eventTypeRedirectUrl");
    });
  });
});
