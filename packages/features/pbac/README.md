...
function TeamSettings({ teamId }: { teamId: number }) {
  ...
  const { hasPermissions } = usePermissions(teamId, ["team.update", "team.invite"]);
...
```typescript
["team.read", "team.update", "custom:team.invite", "custom:team.remove"];
```

### 3. Booking Management with Recordings

```typescript
["booking.read", "booking.update", "custom:booking.readRecordings"];
```
