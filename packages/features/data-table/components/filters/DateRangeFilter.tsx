...
export const DateRangeFilter = ({ column, options, showClearButton = false }: DateRangeFilterProps) => {
  ...
  const [startDate, setStartDate] = useState<Dayjs | undefined>(
    filterValue?.data.startDate ? (dayjs(filterValue.data.startDate) as any) : undefined
  );
  const [endDate, setEndDate] = useState<Dayjs | undefined>(
    filterValue?.data.endDate ? (dayjs(filterValue.data.endDate) as any) : undefined
  );
  ...
  useEffect(() => {
    ...
    if (!filterValue && !forceCustom) {
      updateValues({
        ...
        startDate: getDefaultStartDate() as any,
        endDate: getDefaultEndDate() as any,
      });
    }
  ...
  const updateDateRangeFromPicker = ({
    ...
    updateValues({
      preset: CUSTOM_PRESET,
      startDate: (startDate ? dayjs(startDate) : getDefaultStartDate()) as any,
      endDate: (endDate ? dayjs(endDate) : undefined) as any,
    });
  };
