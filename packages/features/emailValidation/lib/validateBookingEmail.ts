...
import { EmailValidationResponse } from "./service/IEmailValidationService.interface";
...
interface EmailValidationResult {
  providerValidationPromise: Promise<EmailValidationResponse> | null;
  providerError: HttpError | null;
  startProviderValidation(): void;
  waitForProviderValidation(): Promise<void>;
}
...
ValidateBookingEmailParams): Promise<EmailValidationResult | null> {
  ...
  try {
    ...
    const result: EmailValidationResult = {
      providerError: null,
      providerValidationPromise: null,
      startProviderValidation() {
        const providerValidationPromise = (result.providerValidationPromise =
          emailValidationService.validateWithProvider({
            request: { email },
            skipCache: true,
          }));
        providerValidationPromise
          .then((providerResult) => {
          ...
          .catch((error) => {
            if (error instanceof HttpError) {
              ...
              result.providerError = error;
              return;
            }
            logger.error(
              "Provider email validation unhandled error - allowing booking",
              safeStringify(error)
            );
          });
      },
      async waitForProviderValidation() {
        if (!result.providerValidationPromise) {
          return;
        }
        await result.providerValidationPromise;
