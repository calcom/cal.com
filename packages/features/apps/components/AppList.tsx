...
const AppList = ({
  ...
  const ChildAppCard = ({ item }: { item: AppCardApp }): JSX.Element => {
    ...
    <>
      <List className={listClassName}>
        ...
        {data.items
          ...
          .map((item, i) => (
            <ChildAppCard key={item.slug || i} item={item} />
          ))}
      </List>
      {locationType && (
...
function ConnectOrDisconnectIntegrationMenuItem(props: {
  ...
  const renderButton = (buttonProps: Record<string, unknown>): JSX.Element => (
