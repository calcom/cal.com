...
describe("getRawEventType", () => {
  ...
  describe("Organization admin access", () => {
    ...
    test.skip("should fetch user event type when user is org admin and user belongs to org", async () => {
      // note(AI): This test is skipped because getRawEventType currently only supports org admin
      // access for platform organizations (isUserInPlatformOrganization: true),
      // not for regular organizations
      const organization = await createTestOrganization();
      const orgAdmin = await createTestOrgAdmin(organization.id);
      const orgUser = await createTestUser({ organizationId: organization.id, withProfile: true });
      const eventType = await createTestEventType(orgUser.id, { title: "Org User Event Type" });
  ...
  describe("Platform organization behavior", () => {
