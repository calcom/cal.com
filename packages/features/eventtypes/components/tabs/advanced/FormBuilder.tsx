...
function useScrollOnItemAdded(containerRef: React.RefObject<HTMLElement | null>, itemCount: number) {
  const prevCountRef = useRef(itemCount);
  useEffect(() => {
    if (itemCount > prevCountRef.current && containerRef.current) {
...
function Options({
  ...
  useScrollOnItemAdded(scrollContainerRef, value?.length ?? 0);
  ...
  return (
    <div className={className}>
      ...
      <div
        ...
        className="bg-cal-muted max-h-96 overflow-y-auto rounded-md p-4"
