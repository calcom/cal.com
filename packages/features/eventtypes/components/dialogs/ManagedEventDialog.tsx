...
export default function ManagedEventDialog(props: ManagedEventDialogProps) {
  ...
  return (
    <Dialog open={slugExistsChildrenDialogOpen.length > 0} onOpenChange={onOpenChange}>
      <DialogContent type="confirmation" Icon="circle-alert">
        <DialogHeader>
          <DialogTitle>
            {t("managed_event_dialog_title", {
              slug,
        ...
        <DialogFooter className="mt-4">
          <DialogClose />
          <Button color="destructive" onClick={onConfirm} loading={isPending} disabled={isPending}>
            {t("managed_event_dialog_confirm_button", {
              count: slugExistsChildrenDialogOpen.length,
            })}
          </Button>
        </DialogFooter>
