...
const log = logger.getSubLogger({ prefix: ["[features] conferencing"] });

const translator = short();

/**
 * Get busy times from video conferencing providers
 */
export const getBusyVideoTimes = async (withCredentials: CredentialPayload[]) =>
...
/**
 * Create a video meeting using the first available video adapter
 */
export const createMeeting = async (credential: CredentialPayload, calEvent: CalendarEvent) => {
 ...
 */
export const deleteMeeting = async (credential: CredentialPayload | null, uid: string): Promise<unknown> => {
  if (credential) {
...
/**
 * Create a meeting with Cal Video as fallback
...
/**
 * Create an instant meeting with Cal Video
 */
