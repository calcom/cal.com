...
/**
 * BOOKING_REJECTED Trigger Tests
 *
 * Tests specific to the BOOKING_REJECTED webhook trigger.
 * This trigger fires when a booking that required confirmation is rejected by the host.
 *
 * Scenarios covered:
 * - Basic rejection flow
 * - Team/collective scheduling rejection
 * - Integration: real PayloadBuilder verifies REJECTED status in payload
 */
describe("BOOKING_REJECTED Trigger", () => {
  ...
  describe("Rejected booking", () => {
    it("should process BOOKING_REJECTED when host declines a pending booking", async () => {
      const payload: BookingWebhookTaskPayload = {
        operationId: "op-booking-rejected-1",
        triggerEvent: WebhookTriggerEvents.BOOKING_REJECTED,
      ...
      vi.mocked(mockWebhookRepository.getSubscribers).mockResolvedValueOnce([
        {
          ...
          time: null,
          timeUnit: null,
          eventTriggers: [WebhookTriggerEvents.BOOKING_REJECTED],
          version: WebhookVersion.V_2021_10_20,
        },
      ]);

      await consumer.processWebhookTask(payload, "task-booking-rejected-1");

      expect(mockBookingDataFetcher.canHandle).toHaveBeenCalledWith(WebhookTriggerEvents.BOOKING_REJECTED);
    ...
    it("should not send webhook when no subscribers match", async () => {
      const payload: BookingWebhookTaskPayload = {
        operationId: "op-booking-rejected-no-sub",
        triggerEvent: WebhookTriggerEvents.BOOKING_REJECTED,
        bookingUid: "booking-uid-123",
        eventTypeId: 456,
        userId: 789,
  ...
  describe("Team/Collective scheduling", () => {
    it("should process BOOKING_REJECTED for team event", async () => {
      const payload: BookingWebhookTaskPayload = {
        operationId: "op-team-rejected",
        triggerEvent: WebhookTriggerEvents.BOOKING_REJECTED,
      ...
      await consumer.processWebhookTask(payload, "task-team-rejected");
  ...
  describe("Integration: real PayloadBuilder verifies webhook payload content", () => {
    ...
    it("delivers REJECTED status and BOOKING_REJECTED triggerEvent in payload", async () => {
      ...
      expect(envelope.triggerEvent).toBe("BOOKING_REJECTED");
    ...
    it("includes bookingId, title, location, organizer, and attendees from calendar event", async () => {
      ...
      expect(p.title).toBe("Rejected Meeting");
    ...
    it("does not include metadata in BOOKING_REJECTED payload (unlike CREATED/REQUESTED)", async () => {
      vi.mocked(mockWebhookRepository.getSubscribers).mockResolvedValueOnce([defaultSubscriber]);
