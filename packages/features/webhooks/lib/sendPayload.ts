...
const sendPayload = async (
  ...
  // Handle FORM_SUBMITTED_NO_EVENT with delay
...
export const sendGenericWebhookPayload = async ({
  ...
  // If this is a FORM_SUBMITTED_NO_EVENT with delay, schedule it
  if (triggerEvent === "FORM_SUBMITTED_NO_EVENT" && (delayMinutes ?? webhook.delayMinutes)) {
    ...
    // We need to schedule this with the delay
    return {
      ok: true,
      status: 202,
  ...
  // Immediate execution for all other cases
