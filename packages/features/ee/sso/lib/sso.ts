...
export const ssoTenantProduct = async (prisma: PrismaClient, email: string) => {
  ...
  if (!memberships || memberships.length === 0) {
    if (!HOSTED_CAL_FEATURES) throw new AuthorizationError("no_account_exists");

    const domain = email.split("@")[1];
    const organization = await OrganizationRepository.getVerifiedOrganizationByAutoAcceptEmailDomain(domain);

    if (!organization) throw new AuthorizationError("no_account_exists");
    ...
    if (!memberships || memberships.length === 0) throw new AuthorizationError("no_account_exists");
  }
