...
function CallHistoryContent({ org: _org }: CallHistoryProps) {
  ...
  const callHistoryData: CallHistoryRow[] = useMemo(() => {
    ...
    return callsData.calls.map((call) => ({
      ...
      sessionOutcome: call.sessionOutcome || "successful",
    }));
  }, [callsData?.calls]);
...
function CallHistoryPage() {
  ...
  return session.data ? (
    <Shell withoutMain backPath="/workflows">
      <ShellMain heading={t("call_history")} subtitle={t("call_history_subtitle")}>
        <CallHistoryTable org={org} />
      </ShellMain>
    </Shell>
  ) : (
    <></>
