...
export const SetPasswordForm = () => {
    ...
    return (<>
      <Form form={setPasswordFormMethods} handleSubmit={(v) => {
        ...
        <div className="mb-5">
          <Controller name="password" control={setPasswordFormMethods.control} render={({ field: { onBlur, onChange, value } }) => (<PasswordField value={value || ""} onBlur={onBlur} onChange={async (e) => {
                onChange(e.target.value);
                setPasswordFormMethods.setValue("password", e.target.value);
                await setPasswordFormMethods.trigger("password");
            }} hintErrors={["caplow", "admin_min", "num"]} name="password" autoComplete="off"/>)}/>
        </div>

        <div className="flex">
          <Button disabled={setPasswordFormMethods.formState.isSubmitting || setPasswordMutation.isLoading} color="primary" EndIcon={ArrowRight} type="submit" className="w-full justify-center">
            {t("continue")}
          </Button>
        </div>
      </Form>
