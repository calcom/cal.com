...
export const AddNewTeamsForm = () => {
    ...
    return (<>
      {Array.from({ length: counter }, (_, index) => (<div className="relative" key={index}>
          <TextField key={index} value={inputValues[index]} onChange={(e) => handleInputChange(index, e.target.value)} addOnClassname="bg-transparent p-0 border-l-0" addOnSuffix={index > 0 && (<Button color="minimal" className="group/remove mx-2 px-0 hover:bg-transparent" onClick={() => handleRemoveInput(index)}>
                  <X className="bg-subtle text group-hover/remove:text-inverted group-hover/remove:bg-inverted h-5 w-5 rounded-full p-1"/>
                </Button>)}/>
        </div>))}
      <Button StartIcon={Plus} color="secondary" disabled={createTeamsMutation.isLoading} onClick={handleCounterIncrease}>
        {t("add_a_team")}
      </Button>
      <Button EndIcon={ArrowRight} color="primary" className="mt-6 w-full justify-center" disabled={createTeamsMutation.isLoading} onClick={() => {
