...
function createMockRepository(): PrismaSmtpConfigurationRepository {
  return {
    create: vi.fn(),
    findById: vi.fn(),
    ...
    update: vi.fn(),
  } as unknown as PrismaSmtpConfigurationRepository;
}
...
function createService(
  repoOverrides?: Partial<PrismaSmtpConfigurationRepository>,
...
describe("SmtpConfigurationService", () => {
  ...
  describe("create", () => {
    ...
    it("should reject duplicate config for same team", async () => {
      const mockRepo = createMockRepository();
      vi.mocked(mockRepo.isOrganization).mockResolvedValue(true);
      vi.mocked(mockRepo.findByTeamId).mockResolvedValue(makeConfig());
  ...
  describe("delete", () => {
    it("should delete config when it exists", async () => {
      ...
      expect(mockRepo.delete).toHaveBeenCalledWith(TEAM_ID);
    });
  ...
  describe("update", () => {
    it("should update non-credential fields without encrypting", async () => {
      ...
      const service = createService(mockRepo);
      await service.update(TEAM_ID, { fromEmail: "new@org.com" });

      const updateArg = vi.mocked(mockRepo.update).mock.calls[0][1];
    ...
    it("should encrypt credentials when provided", async () => {
      ...
      const service = createService(mockRepo);
      await service.update(TEAM_ID, { smtpUser: "newuser", smtpPassword: "newpass" });
    ...
    it("should throw NotFound when config does not exist", async () => {
      ...
      const service = createService(mockRepo);
      await expect(service.update(TEAM_ID, { fromEmail: "x@y.com" })).rejects.toThrow(
        expect.objectContaining({ code: ErrorCode.NotFound })
      );
    });
  });
  ...
  describe("sendTestEmail", () => {
    it("should decrypt credentials and call smtpService", async () => {
      ...
      const result = await service.sendTestEmail(TEAM_ID, "user@test.com", language);
    ...
    it("should throw NotFound when config does not exist", async () => {
      ...
      const service = createService(mockRepo);
      const language = vi.fn() as unknown as import("i18next").TFunction;
      await expect(service.sendTestEmail(TEAM_ID, "user@test.com", language)).rejects.toThrow(
        expect.objectContaining({ code: ErrorCode.NotFound })
      );
    });
  });
