...
export async function handleExistingUsersInvites({
  ...
  invitableExistingUsers: (UserWithMembership & {
    newRole: MembershipRole;
  })[];
  team: TeamWithParent;
  ...
  if (!team.isOrganization) {
    ...
    if (autoJoinUsers.length) {
      const createdMemberships = await createMemberships({
        teamId,
        language,
      ...
      memberships = memberships.concat(createdMemberships);
      ...
      await sendExistingUserTeamInviteEmails({
        ...
        existingUsersWithMemberships: autoJoinUsers.map((u) => ({ ...u, profile: null })),
    ...
    if (regularUsers.length) {
      ...
      await sendExistingUserTeamInviteEmails({
        currentUserName: inviter.name,
        currentUserTeamName: team?.name,
        existingUsersWithMemberships: regularUsers.map((u) => ({ ...u, profile: null })),
        language: translation,
        isOrg: isOrg,
        teamId: team.id,
        isAutoJoin: false,
...
export async function handleNewUsersInvites({
  ...
  const createdMemberships = createdUsers.flatMap((user) => user.teams);
...
export async function createMemberships({
  ...
  try {
    const userIds = invitees.map((invitee) => invitee.id);
