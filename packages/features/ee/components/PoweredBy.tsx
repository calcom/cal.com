...
const PoweredByCal = ({ logoOnly, username }: { logoOnly?: boolean; username?: string }) => {
  const { t } = useLocale();
  ...
  return (
    <div className={`p-2 text-center text-xs sm:text-right${isEmbed ? " max-w-3xl" : ""}`}>
      <Link
        // replace the long UTM url with a Dub short link (will need to run script to backfill all usernames)
        href={username ? `https://go.cal.com/${username}` : POWERED_BY_URL}
        target="_blank"
