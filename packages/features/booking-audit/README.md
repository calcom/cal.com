...
```typescript
// User confirms booking via web app
{
  source: "WEBAPP",
...
{
  source: "WEBHOOK",
  actor: await makeNamedSystemActor("stripe-payment-webhook", "Stripe Payment Webhook", repo)
}

// API v2 endpoint adds guest
{
  source: "API_V2",
  actor: await getPIIFreeBookingAuditActor({ guestActor: { email, name }, ... })
}
```
