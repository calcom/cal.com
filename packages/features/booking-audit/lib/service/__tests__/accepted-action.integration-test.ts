...
describe("Accepted Action Integration", () => {
  ...
  describe("when single booking is accepted", () => {
    it("should create audit record and retrieve it with correct data formatting", async () => {
      ...
      expect(displayData.previousStatus).toBe(BookingStatus.PENDING);
      expect(displayData.newStatus).toBe(BookingStatus.ACCEPTED);
    });
  ...
  describe("when multiple bookings are accepted in bulk", () => {
    it("should create audit records for all bookings with same operation ID", async () => {
      ...
      expect(result1.auditLogs[0].action).toBe("ACCEPTED");
      expect(result2.auditLogs[0].action).toBe("ACCEPTED");
      expect(result3.auditLogs[0].action).toBe("ACCEPTED");
