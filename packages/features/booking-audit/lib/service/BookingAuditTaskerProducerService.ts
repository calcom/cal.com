...
import type { PiiFreeActor, ActorIdentification, BookingAuditContext } from "../dto/types";
...
export class BookingAuditTaskerProducerService implements BookingAuditProducerService {
  ...
  private async getPIIFreeBookingAuditActor(params: { actor: ActorIdentification }): Promise<PiiFreeActor> {
    const { actor } = params;
