...
export class BookingAuditService {
  ...
  // TODO: This method needs to be updated to accept userUuid instead of userId
  // For now, using system actor as fallback
  private async getOrCreateUserActor(userId: number): Promise<string> {
    // Temporary: Return system actor ID until we update callers to pass userUuid
    logger.warn("getOrCreateUserActor called with userId - using system actor", { userId });
