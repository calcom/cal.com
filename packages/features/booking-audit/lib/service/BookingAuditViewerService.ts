 ...
 * Handles read operations, permission checks, actor enrichment, and display formatting
 * Separated from BookingAuditService which handles write operations
 */
export class BookingAuditViewerService {
    ...
    async getAuditLogsForBooking(
        ...
        // Check permissions
        await this.checkPermissions(bookingUid, userId, userEmail);
     ...
     * User must be: booking owner, attendee, or team admin/owner
     */
    private async checkPermissions(bookingUid: string, userId: number, userEmail: string): Promise<void> {
        ...
        const isAttendee = booking.attendees.some((attendee: { email: string }) => attendee.email === userEmail);
    ...
    private async enrichActorInformation(actor: BookingAuditWithActor["actor"]): Promise<{
        ...
        if (actor.userUuid) {
            const actorUser = await this.prismaClient.user.findUnique({
                where: { uuid: actor.userUuid },
                select: {
                    name: true,
                    email: true,
                },
