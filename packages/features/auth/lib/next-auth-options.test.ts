...
describe("Azure AD JWT callback", () => {
  ...
  describe("profileId regression fix", () => {
    it("uses visitorProfileId from determineProfile as profileId", async () => {
      mockPrismaUserFindFirst.mockResolvedValue({
        id: 1,
        email: "user@example.com",
        name: "Test",
      ...
      expect((result as any).profileId).toBe(42);
    });

    it("falls back to token.profileId when visitorProfileId is null", async () => {
      mockPrismaUserFindFirst.mockResolvedValue({
        id: 1,
        email: "user@example.com",
        name: "Test",
      ...
      const result = await jwtCallback({
        token: { sub: "1", email: "user@example.com", profileId: 99 } as any,
        user: {
          id: "1",
          email: "user@example.com",
          name: "Test",
      ...
      expect((result as any).profileId).toBe(99);
    });

    it("falls back to null when both visitorProfileId and token.profileId are missing", async () => {
      mockPrismaUserFindFirst.mockResolvedValue({
        id: 1,
        email: "user@example.com",
        name: "Test",
      ...
      expect((result as any).profileId).toBeNull();
    });
  });
});
