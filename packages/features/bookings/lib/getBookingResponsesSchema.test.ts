...
describe("getBookingResponsesSchema", () => {
  ...
  describe("select field type", () => {
    test("should successfully parse a select type field", async () => {
      ...
      expect(parsedResponses.data.testSelect).toBe("option1");
    });
  });

  describe("textarea field type", () => {
    test("should successfully parse a textarea field", async () => {
      const schema = getBookingResponsesSchema({
        bookingFields: [
          { name: "name", type: "name", required: true },
          { name: "email", type: "email", required: true },
      ...
      const parsedResponses = await schema.safeParseAsync({
        email: "test@test.com",
        name: "test",
        testTextarea: "Some long text content",
      });
      expect(parsedResponses.success).toBe(true);
      if (!parsedResponses.success) throw new Error("Should not reach here");
      expect(parsedResponses.data.testTextarea).toBe("Some long text content");
    });
  });

  describe("number field type", () => {
    test("should successfully parse a number field", async () => {
      const schema = getBookingResponsesSchema({
        bookingFields: [
          { name: "name", type: "name", required: true },
          { name: "email", type: "email", required: true },
      ...
      expect(parsedResponses.success).toBe(true);
      if (!parsedResponses.success) throw new Error("Should not reach here");
      expect(parsedResponses.data.testNumber).toBe("123");
    });
  });

  describe("checkbox field type", () => {
    test("should successfully parse a checkbox field", async () => {
      const schema = getBookingResponsesSchema({
        bookingFields: [
          { name: "name", type: "name", required: true },
          { name: "email", type: "email", required: true },
      ...
      expect(parsedResponses.success).toBe(true);
      if (!parsedResponses.success) throw new Error("Should not reach here");
      expect(parsedResponses.data.testCheckbox).toEqual(["option1", "option2"]);
    });
  });

  describe("radio field type", () => {
    test("should successfully parse a radio field", async () => {
      const schema = getBookingResponsesSchema({
        bookingFields: [
          { name: "name", type: "name", required: true },
          { name: "email", type: "email", required: true },
      ...
      expect(parsedResponses.success).toBe(true);
      if (!parsedResponses.success) throw new Error("Should not reach here");
      expect(parsedResponses.data.testRadio).toBe("option1");
    });
  });

  describe("boolean field type", () => {
    test("should successfully parse a boolean field", async () => {
      const schema = getBookingResponsesSchema({
        bookingFields: [
          { name: "name", type: "name", required: true },
          { name: "email", type: "email", required: true },
      ...
      expect(parsedResponses.success).toBe(true);
      if (!parsedResponses.success) throw new Error("Should not reach here");
      expect(parsedResponses.data.testBoolean).toBe(true);
    });

    test("should successfully parse a boolean field string 'true'", async () => {
      const schema = getBookingResponsesSchema({
        bookingFields: [
          { name: "name", type: "name", required: true },
          { name: "email", type: "email", required: true },
