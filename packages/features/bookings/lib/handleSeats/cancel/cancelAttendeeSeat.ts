...
async function cancelAttendeeSeat(
  ...
  const { evt, eventTypeInfo } = dataForWebhooks;
  ...
  // Send webhook for attendee seat cancellation
  try {
    ...
    logger.error("Error sending webhook for attendee seat cancellation", error);
  }
