...
const log = logger.getSubLogger({ prefix: ["[handleBookingRequestedNew] book:user"] });

/**
 * Handles booking requested events using the new webhook architecture
 * This is a migrated version of handleBookingRequested that uses the new webhook system
 */
export async function handleBookingRequestedNew(args: {
  evt: CalendarEvent;
  booking: {
    eventType: {
      team?: {
  ...
  try {
    ...
    log.error("Error in handleBookingRequestedNew", safeStringify(error));
  }
}
