...
export const handleNewRecurringBooking = async (input: BookingHandlerInput): Promise<BookingResponse[]> => {
  ...
  */
  ...
  const appsStatus: AppsStatus[] | undefined = undefined;
  const numSlotsToCheckForAvailability = 1;
  const thirdPartyRecurringEventId = null;
  ...
  // --- Handle Round Robin (temporarily skipped) ---
  if (isRoundRobin) {
    throw new Error("Round robin recurring booking handling not implemented yet.");
  }

  // --- Handle 1-1 recurring booking (single record) ---
  const bookingInfo = {
    ...firstBooking,
    appsStatus,
    allRecurringDates: undefined, // not needed anymore
    isFirstRecurringSlot: true,
    thirdPartyRecurringEventId,
    numSlotsToCheckForAvailability,
    currentRecurringIndex: 0,
