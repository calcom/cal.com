...
/**
 * Alias for makeGuestActor for attendee scenarios
 */
export const makeAttendeeActor = makeGuestActor;
...
export function actorToAuditString(actor?: Actor): string | null {
  ...
  switch (actor.identifiedBy) {
    ...
    case "user":
      return `User:${actor.userUuid}`;
    case "attendee":
      return `Attendee:${actor.attendeeId}`;
    case "guest":
      return actor.email ? `Guest:${actor.email}` : "Guest";
    default:
      return null;
  }
}
