...
async function handler(
  ...
  if (originalRescheduledBooking?.uid) {
    log.debug("Rescheduling booking:", originalRescheduledBooking.uid);
    ...
    if (results.length > 0 && results.some((res) => !res.success)) {
      ...
      if (results.length) {
        ...
        if (updatedEvent) {
          ...
          handleAppsStatus(results, booking, reqAppsStatus, evt);
    ...
    log.debug(
      "Booking doesn't require confirmation and payment. Go ahead with making calendar bookings, generating video urls, sending scheduled emails, workflow triggers"
    );
    ...
    log.debug(
      "Booking requires confirmation or it requires payment. So, we can't book calendars, generate video links etc"
    );
  }
  ...
  // Don't send these emails in case of payment requirement as in that case first the payment needs to be done for which separate email is sent/
  if (!isConfirmedByDefault && noEmail !== true && !bookingRequiresPayment) {
    log.debug("Sending Organizer and Attendee Request emails");
  ...
  if (bookingRequiresPayment) {
    log.debug(
      "Booking requires payment so we will generate a payment link here and requesting a redirect to that"
    );
  ...
  if (isConfirmedByDefault) {
    ...
    // FIXME: It should have used requiresConfirmation variable as that takes care of threshold based requires confirmation.
  ...
  if (rescheduleUid) {
    ...
    if (!bookingSeat) {
      ...
      if (!newTimeSlotBooking) {
        ...
        addVideoCallDataToEvt(newBooking.references, evt);
        ...
        if (results.length > 0 && results.some((res) => !res.success)) {
          ...
          if (results.length) {
            ...
            if (updatedEvent) {
              ...
              metadata.conferenceData = updatedEvent.conferenceData;
              metadata.entryPoints = updatedEvent.entryPoints;
              handleAppsStatus(results, newBooking, reqAppsStatus, evt);
            }
          }
        }
        ...
        addVideoCallDataToEvt(updatedNewBooking.references, evt);
