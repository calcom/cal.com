...
import { getLuckyUser, getMultipleLuckyUsers } from "@calcom/lib/server/getLuckyUser";
...
async function handler(
  ...
  if (isFirstSeat) {
    ...
    if (!input.bookingData.allRecurringDates || input.bookingData.isFirstRecurringSlot) {
      ...
      // Get the number of hosts to select from round-robin pool
      const roundRobinCount = eventType.roundRobinCount || 1;

      // If we need multiple hosts, use the new function
      if (roundRobinCount > 1) {
        ...
        // Get multiple lucky users based on roundRobinCount
        const multipleLuckyUsers = await getMultipleLuckyUsers({
          availableUsers: luckyUserPool as [(typeof luckyUserPool)[0], ...(typeof luckyUserPool)[0][]],
          allRRHosts: rrHosts,
          eventType,
          routingFormResponse,
          roundRobinCount, // pass the count to select multiple hosts
        });

        // Check availability for recurring event
        if (
          input.bookingData.isFirstRecurringSlot &&
          eventType.schedulingType === SchedulingType.ROUND_ROBIN
        ) {
          // Check if all selected hosts are available for future slots
          for (const luckyUser of multipleLuckyUsers) {
            try {
              let isAvailableForAllSlots = true;
              for (
                let i = 0;
                i < input.bookingData.allRecurringDates.length &&
                i < input.bookingData.numSlotsToCheckForAvailability;
                ...
                try {
                  await ensureAvailableUsers(
                    { ...eventTypeWithUsers, users: [luckyUser] },
                    {
                      dateFrom: dayjs(start).tz(reqBody.timeZone).format(),
                      dateTo: dayjs(end).tz(reqBody.timeZone).format(),
                      timeZone: reqBody.timeZone,
              ...
              if (isAvailableForAllSlots) {
                luckyUsers.push(luckyUser);
                if (luckyUsers.length >= roundRobinCount) {
                  break; // We have enough hosts
                }
              } else {
                notAvailableLuckyUsers.push(luckyUser);
                loggerWithEventDetails.info(
                  `Round robin host ${luckyUser.name} not available for all slots. Skipping.`
                );
              }
            } catch {
              notAvailableLuckyUsers.push(luckyUser);
          ...
          // For non-recurring events, just add all the lucky users
          luckyUsers.push(...multipleLuckyUsers);
        }
      } else {
        // Original logic for single host round-robin
