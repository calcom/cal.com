...
async function handler(
  ...
  // Check spam for organization-specific events early (before any booking work)
  // Get organization ID from either team (for team events) or event owner (for personal events)
  const team = eventType.team ?? eventType.parent?.team ?? null;
  let eventOrganizationId = team?.parentId ?? null;
  console.log("event:", JSON.stringify(eventType, null, 2));
  // If not a team event, get org ID from event owner
  if (!eventOrganizationId && eventType.userId) {
    ...
    eventOrganizationId = eventOwner?.profiles[0]?.organizationId ?? null;
  }
  

  const spamCheckService = getSpamCheckService();
  await spamCheckService.startCheck({ email: bookerEmail, organizationId: eventOrganizationId ?? null });
