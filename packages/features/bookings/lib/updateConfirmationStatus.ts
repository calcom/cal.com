...
interface ConfirmationStatusOptions {
  user: {
    id: number;
    email: string;
...
export const updateConfirmationStatus = async ({ user, data }: ConfirmationStatusOptions) => {
  const { bookingId, recurringEventId, reason: rejectionReason, confirmed } = data;
