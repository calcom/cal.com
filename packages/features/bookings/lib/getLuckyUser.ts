    ...
    const usersBusyTimesQuery = await Promise.all(
      usersWithCredentials.map((user) =>
        getBusyCalendarTimes({
          ...
          dateFrom: getIntervalStartDate({ interval, rrTimestampBasis, meetingStartTime }).toISOString(),
          dateTo: getIntervalEndDate({ interval, rrTimestampBasis, meetingStartTime }).toISOString(),
          selectedCalendars: user.userLevelSelectedCalendars,
          mode: "slots",
