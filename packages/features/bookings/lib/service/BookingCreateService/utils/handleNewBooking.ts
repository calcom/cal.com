...
export class BookingCreateService {
  ...
  private async handler(
    ...
    const { prisma, bookingRepository, cacheService, checkBookingAndDurationLimitsService } = this.deps;
...
export type HandleNewBookingResponse = Awaited<ReturnType<BookingCreateService["create"]>>;

// TODO: Backward compatibility handler => Required for unit tests. To be removed in a followup PR.
export default async function handler(
  input: BookingHandlerInput,
  bookingDataSchemaGetter?: BookingDataSchemaGetter
) {
  // Impor  t here to avoid circular dependency(BookingFactory -> bookingCreateService -> BookingFactory)
  const { getBookingCreateFactory } = await import("@calcom/lib/di/containers/BookingCreateFactory");
  const factory = getBookingCreateFactory();
