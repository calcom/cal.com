...
describe("BookingCreateFactory", () => {
  ...
  describe("createBooking", () => {
    it("should call bookingCreateService.handle with correct parameters", async () => {
      ...
      vi.mocked(mockBookingCreateService.create).mockResolvedValue(mockResult);

      const result = await factory.createBooking({
        bookingData: mockBookingData,
      });

      expect(mockBookingCreateService.create).toHaveBeenCalledWith({
        bookingData: mockBookingData,
        bookingMeta: undefined,
        schemaGetter: undefined,
      });
