...
export class BookingRepository {
  ...
  /**
   * Gets booking counts per user for round-robin partial loading.
   * This is a fast DB query used to rank hosts by booking shortfall
   * before fetching their calendar availability.
   */
  async getBookingCountsPerUser({
    eventTypeId,
    userIds,
    startDate,
    endDate,
    ...
    const bookings = await this.prismaClient.booking.groupBy({
      by: ["userId"],
      where: {
        eventTypeId,
        status: BookingStatus.ACCEPTED,
    ...
    for (const booking of bookings) {
      if (booking.userId !== null) {
        countMap.set(booking.userId, booking._count.id);
      }
    }
