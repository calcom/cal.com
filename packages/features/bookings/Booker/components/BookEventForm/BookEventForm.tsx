...
type BookEventFormProps = {
  ...
  lastActiveBooking?: { id: number; date: string; uid: string };
};
...
export const BookEventForm = ({
  ...
  classNames,
  timeslot,
  hasMultipleActiveBookings = false,
  lastActiveBooking,
        ...
        <div className="modalsticky mt-auto flex justify-end space-x-2 rtl:space-x-reverse">
          {isInstantMeeting ? (
            ...
            <>
              ...
              {hasMultipleActiveBookings && lastActiveBooking ? (
                <div className="flex items-center space-x-2">
                  <Alert
                    severity="info"
                    message={t("only_last_booking_reschedule_message", {
                      date: formatEventFromTime({
                        date: lastActiveBooking.date,
                        timeFormat,
                        timeZone: timezone,
                        language: i18n.language,
                      }).date,
...
const getError = ({
  ...
  if (
    ...
    if (error?.data?.previousDate) {
      const formattedDate = formatEventFromTime({
        date: error.data.previousDate,
        timeFormat,
        timeZone: timezone,
        language,
      });
      const previousDate = `${formattedDate.date} ${formattedDate.time}`;
      return <>{t("your_previous_booking_has_been_rescheduled", { previousDate })}</>;
    }
    return <>{t("your_previous_booking_has_been_rescheduled")}</>;
  }
