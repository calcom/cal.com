...
export type BookerStore = {
  ...
  /**
   * For multi-booking: Array of selected timeslots.
   */
  selectedTimeslots: string[];
  addSelectedTimeslot: (timeslot: string) => void;
  removeSelectedTimeslot: (timeslot: string) => void;
...
export const createBookerStore = () =>
  createWithEqualityFn<BookerStore>((set, get) => ({
    ...
    selectedTimeslots: [],
    addSelectedTimeslot: (timeslot: string) => {
      const current = get().selectedTimeslots;
      if (!current.includes(timeslot)) {
        set({ selectedTimeslots: [...current, timeslot] });
      }
    },
    removeSelectedTimeslot: (timeslot: string) => {
      const current = get().selectedTimeslots;
      set({ selectedTimeslots: current.filter((t) => t !== timeslot) });
    },
    formValues: {},
