...
export const Day = ({
  ...
  return (
    <button
      ...
      {date.date()}
      {date.isToday() && (
        <span className="absolute bottom-0 left-0 right-0 h-2/5 align-middle text-4xl leading-[0rem]">.</span>
      )}
    </button>
  );
};
      ...
      {days.map((day, idx) => (
        <div key={day === null ? `e-${idx}` : `day-${day.format()}`} className="relative w-full pt-[100%]">
          {day === null ? (
            ...
            <button
              className=" bg-muted text-muted opcaity-50 absolute bottom-0 left-0 right-0 top-0 mx-auto flex w-full items-center justify-center rounded-sm border-transparent text-center"
              key={`e-${idx}`}
              disabled>
