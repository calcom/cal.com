    ...
    <button
      ...
      className={classNames(
        ...
        active
          ...
          : !disabled
          ? "dark:bg-darkgray-200 hover:border-brand-default dark:hover:border-darkmodebrand bg-subtle dark:text-inverted"
          : ""
      )}
      data-testid="day"
      ...
      {days.map((day, idx) => (
        <div key={day === null ? `e-${idx}` : `day-${day.format()}`} className="relative w-full pt-[100%]">
          {day === null ? (
            ...
            <button
              className="dark:bg-darkgray-200 bg-mutedext-center text-muted dark:text-muted absolute top-0 left-0 right-0 bottom-0 mx-auto flex w-full items-center justify-center rounded-sm border-transparent opacity-50"
              key={`e-${idx}`}
              disabled>
