...
import { UserUnblockService } from "./UserUnblockService";
import type { IWatchlistBlockLookup } from "./UserUnblockService";
...
type Deps = {
  ...
  globalWatchlistRepo: IWatchlistBlockLookup;
...
export class AdminWatchlistOperationsService extends WatchlistOperationsService {
  ...
  private userUnblockService: UserUnblockService;

  constructor(deps: Deps) {
    super(deps);
    this.userUnblockService = new UserUnblockService({
      watchlistRepo: deps.globalWatchlistRepo,
  ...
  async bulkDeleteWatchlistEntries(
    ...
    await Promise.all(signupEmailEntries.map((entry) => this.userUnblockService.unblockByEmail(entry.value)));
  ...
  async deleteWatchlistEntry(input: DeleteWatchlistEntryInput): Promise<DeleteWatchlistEntryResult> {
    ...
    if (entry.source === WatchlistSource.SIGNUP && entry.type === WatchlistType.EMAIL) {
      await this.userUnblockService.unblockByEmail(entry.value);
    }
