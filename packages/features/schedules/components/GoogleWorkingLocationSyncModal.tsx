      ...
      <DialogContent>
        ...
        {isLoadingCredentials ? (
          <div class="flex items-center justify-center py-8">
            <Icon name="loader" class="text-subtle h-6 w-6 animate-spin" />
          </div>
        ) : !hasGoogleCalendar ? (
          <div class="py-6 text-center">
            <Icon name="calendar" class="text-subtle mx-auto mb-4 h-12 w-12" />
            <p class="text-default mb-4">{t("no_google_calendar_connected")}</p>
          ...
          <Form form={form} handleSubmit={onSubmit}>
            <div class="space-y-4">
              ...
              <div>
                <Label class="mb-2">{t("sync_when_working_from")}</Label>
                <p class="text-subtle mb-3 text-sm">{t("select_location_types_to_sync")}</p>
                <div class="space-y-2">
                  {locationTypeOptions.map((option) => (
                    <Controller
                      ...
                      control={form.control}
                      render={({ field }) => (
                        <label class="border-subtle hover:bg-muted flex cursor-pointer items-start gap-3 rounded-md border p-3 transition-colors">
                          <Checkbox
                          ...
                          />
                          <div>
                            <p class="text-default font-medium">{option.label}</p>
                            <p class="text-subtle text-sm">{option.description}</p>
                          </div>
                        </label>
                      )}
                    />
              ...
              <div class="bg-subtle rounded-md p-3">
                <p class="text-subtle text-sm">
                  <Icon name="info" class="mr-1 inline h-4 w-4" />
