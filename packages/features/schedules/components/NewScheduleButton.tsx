...
import { Button, Dialog, DialogClose, DialogContent, DialogFooter, DialogTrigger, Form, InputField, showToast, } from "@calcom/ui";
import { Plus } from "@calcom/ui/components/icon";

export function NewScheduleButton({ name = "new-schedule", fromEventType, }: {
    name?: string;
    ...
    return (<Dialog name={name} clearQueryParamsOnClose={["copy-schedule-id"]}>
      ...
      <DialogContent title={t("add_new_schedule")}>
        <Form form={form} handleSubmit={(values) => {
          ...
          <InputField label={t("name")} type="text" id="name" required placeholder={t("default_schedule_name")} {...register("name")}/>
          <DialogFooter>
            <DialogClose />
