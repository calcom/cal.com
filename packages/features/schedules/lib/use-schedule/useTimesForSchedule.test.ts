...
describe.skipIf(!isTimezoneSupported())("useTimesForSchedule", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  describe("when using MONTH_VIEW layout", () => {
    ...
    describe("and when current date is after mid-month", () => {
      describe("and when viewing current month", () => {
        it.skip("with a specific selected date, should return time range that prefetches the next month(thus total 2 months) ", () => {
          const currentMonth = "2024-01";
