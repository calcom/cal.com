...
type UseTimesForScheduleProps = Pick<UseScheduleWithCacheArgs, "month" | "dayCount" | "selectedDate"> & {
  bookerLayout: {
    layout: string;
    extraDays: number;
...
export const useTimesForSchedule = ({
  ...
  // Get values from booker store
  const [monthFromStore, bookerState] = useBookerStoreContext((state) => [state.month, state.state], shallow);

  const resolvedMonth = monthFromStore ?? month ?? null;
  const date = dayjs(selectedDate).format("YYYY-MM-DD");

  const { pretchFutureMonths, monthsToPrefetch } = usePrefetch({
    date,
    month: resolvedMonth,
    bookerLayout,
    bookerState,
  ...
  const now = dayjs();
  const monthDayjs = resolvedMonth ? dayjs(resolvedMonth) : now;
  const lastMonthToPrefetchDayjs = monthDayjs.add(monthsToPrefetch, "month");
  ...
  if (!!dayCount && dayCount > 0) {
    ...
    startTime = monthDayjs.startOf("month").toISOString();
    endTime = (pretchFutureMonths ? lastMonthToPrefetchDayjs : monthDayjs).endOf("month").toISOString();
  }
