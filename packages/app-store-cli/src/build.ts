...
function generateFiles() {
  ...
  // Generate video adapters service map for dynamic imports
  ...
  videoOutput.push(
    ...
    "*/",
    "",
    "export const VideoAdapterMap = {"
  );

  forEachAppDir((app) => {
    const libPath = path.join(APP_STORE_PATH, app.path, "lib");
    const videoAdapterPath = path.join(libPath, "VideoApiAdapter.ts");

    if (fs.existsSync(videoAdapterPath)) {
      const normalizedAppName = app.name.replace(/[-.]/g, "");
      const importPath = `./${app.path.replace(/\\/g, "/")}/lib/VideoApiAdapter`;
      videoOutput.push(`  "${normalizedAppName}": import("${importPath}"),`);
    }
  }, isVideoApp);
