...
function generateFiles() {
  ...
  const paymentExportLineIndex = paymentServices.findIndex((line) =>
    line.startsWith("export const PaymentServiceMap")
  );
  if (paymentExportLineIndex !== -1) {
    const exportLine = paymentServices[paymentExportLineIndex];
    const objectContent = paymentServices.slice(paymentExportLineIndex + 1, -1);
