...
function generateFiles() {
  ...
  const oauthScopesOutput: string[] = [];
  oauthScopesOutput.push(`import type { AccessScope } from "@calcom/prisma/enums";`);
  oauthScopesOutput.push(``);
  oauthScopesOutput.push(`export type AppOAuthScopes = {`);
  oauthScopesOutput.push(`  allowed: AccessScope[];`);
  oauthScopesOutput.push(`  required?: AccessScope[];`);
  oauthScopesOutput.push(`};`);
  oauthScopesOutput.push(``);
  oauthScopesOutput.push(`/**`);
  oauthScopesOutput.push(` * Static registry mapping app slug to allowed OAuth scopes`);
  oauthScopesOutput.push(` * Used for per-client scope validation in OAuth provider`);
  oauthScopesOutput.push(` */`);
  oauthScopesOutput.push(`export const appOAuthScopesRegistry: Record<string, AppOAuthScopes> = {`);

  forEachAppDir((app) => {
    if (app.calOAuthScopes) {
      const allowedScopes = JSON.stringify(app.calOAuthScopes.allowed);
      const requiredScopes = app.calOAuthScopes.required
        ? `, required: ${JSON.stringify(app.calOAuthScopes.required)}`
        : "";
      oauthScopesOutput.push(`  "${app.slug || app.name}": { allowed: ${allowedScopes}${requiredScopes} },`);
    }
  });
...
`;
  const filesToGenerate: [string, string[]][] = [
    ...
    ["oauth.scopes.generated.ts", oauthScopesOutput],
  ];
