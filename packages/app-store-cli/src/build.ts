...
function generateFiles() {
  ...
  const kBarAppDirectoryData: [{ slug: string; name: string }] = [];

  forEachAppDir((app) => {
    if (!app?.isTemplate) {
      kBarAppDirectoryData.push({ name: app.appName, slug: app.slug });
    }
  });

  kbarAppRegistryOutput.push(`export const kbarAppRegistry = ${JSON.stringify(kBarAppDirectoryData)}`);
...
`;
  ...
  const filesToGenerate: [string, string[]][] = [
    ...
    ["kbar-app-registry.generated.ts", kbarAppRegistryOutput],
  ];
