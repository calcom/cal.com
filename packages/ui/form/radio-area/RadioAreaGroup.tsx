  ...
  ({ children, className, ...props }, ref) => {
    return (
      <label className={classNames("relative flex", className)}>
        <input
          ref={ref}
          className="text-emphasis bg-subtle border-emphasis focus:ring-none peer absolute left-3 top-[0.9rem] align-baseline"
          type="radio"
          {...props}
        />
        <div className="text-default peer-checked:border-emphasis border-subtle rounded-md border p-4 pl-10 pt-3">
          {children}
        </div>
      </label>
    );
