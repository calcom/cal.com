...
function PhoneInput({ name, className = "", onChange, ...rest }: PhoneInputProps) {
  ...
  return (
    <BasePhoneInput
      ...
      className={`${className} focus-within:border-brand block w-full rounded-md rounded-sm border border border-gray-300 py-px pl-3 ring-black focus-within:ring-1 disabled:text-gray-500 disabled:opacity-50 dark:text-white dark:selection:bg-green-500 disabled:dark:text-gray-500`}
    />
  );
}
