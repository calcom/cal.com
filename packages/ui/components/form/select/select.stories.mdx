...
import { MultiSelect } from "./SelectDownshift";
...
<Canvas>
  <Story name="Combobox downshift">
    <MultiSelect label="Team Select" items={options} labelKey="label" searchableKey="label" />
  </Story>
</Canvas>
...
<Canvas>
  <Story name="Combobox downshift custom selected node">
    <MultiSelect
      ...
      searchableKey="label"
      onSelectedItemsChange={(items) => console.log(items)}
      listItemNode={(selectedItems, getSelectedItemProps, removeSelectedItem) => {
        return (
          <div
