  ...
  return (
    <button
      className={classNames(
        "hover:border-brand disabled:text-bookinglighter absolute top-0 left-0 right-0 bottom-0 mx-auto w-full rounded-sm border border-transparent text-center font-medium text-sm disabled:cursor-default disabled:border-transparent disabled:font-light dark:hover:border-white disabled:dark:border-transparent",
        active
...
const DatePicker = ({
  ...
  return (
    <div className={className}>
      <div className="mb-4 flex justify-between text-xl font-light px-4 sm:px-0">
        <span className="w-1/2 dark:text-white">
          {browsingDate ? (
            <>
              <span className="text-bookingdarker text-base font-semibold dark:text-white">{browsingDate.format("MMMM")},</span>{" "}
              <span className="text-bookinglight font-medium text-base">{browsingDate.format("YYYY")}</span>
            </>
          ) : (
