...
export function GettingStarted({
  ...
  const navigateToStep = (actionKey: Step["actionKey"]) => {
    const routes: Record<Step["actionKey"], () => void> = {
      ...
      updateUsername: () => router.push("/settings/profile"),
      addOrEditEvents: () => router.push("/event-types"),
      setAvailability: () => router.push("/availability"),
      shareYourCalID: () => {
        navigator.clipboard.writeText(`${WEBAPP_URL}${username ? `/${username}` : ""}`);
        triggerToast("Public URL copied!", "success");
      },
    };
  ...
  return (
    <div className="border-default flex w-full flex-col items-center overflow-hidden rounded-3xl border px-4 py-6 shadow-md sm:py-8">
