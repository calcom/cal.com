...
interface RescheduledNotificationConfig {
  previewMode: boolean;
...
const buildSubjectMessage = (eventTitle: string, eventDate: string): string => {
  return `Rescheduled: ${eventTitle} - ${eventDate}`;
};

const createWelcomeSection = (userName?: string): string => {
  const personalizedGreeting = userName ? ` ${userName}` : "";
  return `<body>Hi${personalizedGreeting},<br><br>This event has been rescheduled to a new time.<br><br>`;
};
...
const generateNewScheduleBlock = (eventDate: string, endTime: string, timezone: string): string => {
  return `<div><strong class="editor-text-bold">New Date & Time: </strong></div>${eventDate} - ${endTime} (${timezone})<br><br>`;
};
...
const createFooterSection = (brandingHidden?: boolean, previewMode?: boolean): string => {
  ...
  return `This reschedule notification was triggered by a Workflow in Cal ID.${brandingText}</body>`;
};

const assembleNotificationContent = (
  welcomeHtml: string,
  eventDetailsHtml: string,
  newScheduleHtml: string,
  participantsHtml: string,
  venueHtml: string,
  footerHtml: string
): string => {
  return welcomeHtml + eventDetailsHtml + newScheduleHtml + participantsHtml + venueHtml + footerHtml;
};

const processRescheduledConfiguration = (config: RescheduledNotificationConfig): NotificationOutput => {
  ...
  const newScheduleSection = generateNewScheduleBlock(
...
const emailRescheduledTemplate = ({
  isEditingMode,
  locale,
  action,
  timeFormat,
  ...
  const rescheduledConfig: RescheduledNotificationConfig = {
  ...
  return processRescheduledConfiguration(rescheduledConfig);
};

export default emailRescheduledTemplate;
