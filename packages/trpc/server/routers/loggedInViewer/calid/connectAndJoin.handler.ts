...
import type { TConnectAndJoinInputSchema } from "./calid/connectAndJoin.schema";
...
export const Handler = async ({ ctx, input }: Options) => {
  ...
  const evt: CalendarEvent = {
    ...
    team: !!updatedBooking.eventType?.calIdTeam
      ? {
          name: updatedBooking.eventType.calIdTeam.name,
          id: updatedBooking.eventType.calIdTeam.id,
          members: [],
        }
      : undefined,
  };
  ...
  await scheduleNoShowTriggers({
    ...
    triggerForUser: !eventType?.calIdTeamId || (eventType?.calIdTeamId && eventType?.parentId),
