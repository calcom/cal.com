...
export const updateHandler = async ({ ctx, input }: UpdateOptions) => {
  ...
  const eventType = await ctx.ctx.prisma.eventType.findUniqueOrThrow({
    where: { id },
    select: {
      title: true,
      description: true,
  ...
  if (schedule) {
    ...
    const userScheduleQuery = await ctx.ctx.prisma.schedule.findFirst({
      where: {
        userId: ctx.user.id,
        id: schedule,
      },
  ...
  if (input.metadata?.disableStandardEmails?.confirmation) {
    ...
    const workflows = await ctx.ctx.prisma.workflow.findMany({
      where: {
        activeOn: {
          some: {
  ...
  const connectedLinks = await ctx.ctx.prisma.hashedLink.findMany({
    where: {
      eventTypeId: input.id,
    },
    select: {
  ...
  if (multiplePrivateLinks && multiplePrivateLinks.length > 0) {
    ...
    if (singleLinksToBeDeleted.length > 0) {
      await ctx.ctx.prisma.hashedLink.deleteMany({
        where: {
          eventTypeId: input.id,
          link: {
    ...
    if (multiplePrivateLinksToBeInserted.length > 0) {
      await ctx.ctx.prisma.hashedLink.createMany({
  ...
  if (aiPhoneCallConfig) {
    if (aiPhoneCallConfig.enabled) {
      await ctx.ctx.prisma.aIPhoneCallConfiguration.upsert({
        where: {
          eventTypeId: id,
        },
        update: {
      ...
      await ctx.ctx.prisma.aIPhoneCallConfiguration.delete({
        where: {
          eventTypeId: id,
        },
      });
  ...
  try {
    updatedEventType = await ctx.ctx.prisma.eventType.update({
      where: { id },
      data,
