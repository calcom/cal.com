...
export const updateHandler = async ({ ctx, input }: UpdateOptions) => {
  try {
    ...
    if (input.teamId && eventType.team?.id && input.teamId !== eventType.team.id) {
      throw new AuthorizationError("Unauthorized to update event type for different team");
    }
    ...
    if (finalSeatsPerTimeSlot && finalRecurringEvent) {
      throw new ValidationError("Recurring Events and Offer Seats cannot be active at the same time.");
    }
    ...
    if (bookingLimits) {
      ...
      if (!isValid) throw new ValidationError("Booking limits must be in ascending order.");
      data.bookingLimits = bookingLimits;
    }

    if (maxActiveBookingsPerBooker) {
      if (maxActiveBookingsPerBooker && maxActiveBookingsPerBooker < 1) {
        throw new ValidationError("Booker booking limit must be greater than 0.");
      }
    ...
    if (durationLimits) {
      ...
      if (!isValid) throw new ValidationError("Duration limits must be in ascending order.");
      data.durationLimits = durationLimits;
    }

    if (offsetStart !== undefined) {
      if (offsetStart < 0) {
        throw new ValidationError("Offset start time must be zero or greater.");
      }
    ...
    if (bookerLayoutsError) {
      ...
      throw new ValidationError(t(bookerLayoutsError));
    }
    ...
    if (restrictionScheduleId) {
      ...
      if (restrictionSchedule?.userId !== ctx.user.id) {
        if (!teamId || !restrictionSchedule) {
          throw new AuthorizationError("The restriction schedule is not owned by you or your team");
        }
    ...
    if (teamId && hosts) {
      ...
      if (!hosts.every((host) => teamMemberIds.includes(host.userId)) && !eventType.team?.parentId) {
        throw new AuthorizationError("Some hosts are not team members");
      }
    ...
    try {
      ...
      if (e instanceof Prisma.PrismaClientKnownRequestError) {
        if (e.code === "P2002") {
          ...
          throw new ValidationError("error_event_type_url_duplicate");
        }
      }
      throw e;
    }
