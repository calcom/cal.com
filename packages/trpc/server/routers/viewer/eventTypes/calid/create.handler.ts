...
export const createHandler = async ({ ctx, input }: CreateOptions) => {
  ...
  if (calIdTeamId && schedulingType) {
    ...
    if (
      !isSystemAdmin &&
      !isOrgAdmin &&
      (!hasCalIdMembership?.role || !["ADMIN", "OWNER"].includes(hasCalIdMembership.role))
    ) {
  ...
  // If we are in an organization & they are not admin & they are not creating an event on a calIdTeamId
  // Check if evenTypes are locked.
  if (ctx.user.organizationId && !ctx.user?.organization?.isOrgAdmin && !calIdTeamId) {
  ...
  try {
    ...
    if (!calIdTeamId && isPrismaObjOrUndefined(ctx.user.metadata)?.connectedChatAccounts) {
