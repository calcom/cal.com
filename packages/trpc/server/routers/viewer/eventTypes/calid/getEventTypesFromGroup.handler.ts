...
import type { TCalIdGetEventTypesFromGroupSchema } from "./getByViewer.schema";
...
type GetByViewerOptions = {
  ...
  input: TCalIdGetEventTypesFromGroupSchema;
};
...
export const getEventTypesFromGroup = async ({
  ...
  const { calIdTeamId, parentId } = group;

  const isFilterSet = (filters && hasFilter(filters)) || !!calIdTeamId;
  ...
  if (shouldListUserEvents || !calIdTeamId) {
  ...
  if (calIdTeamId) {
    const calIdTeamEventTypes =
      (await eventTypeRepo.findCalIdTeamEventTypes({
        calIdTeamId,
        userId: ctx.user.id,
        limit,
        cursor,
    ...
    eventTypes.push(...calIdTeamEventTypes);
  }
  ...
  if (calIdTeamId) {
    ...
    if (calIdMembership && calIdMembership.calIdTeam.isTeamPrivate)
