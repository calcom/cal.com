  ...
  try {
    ...
    if (teamId && schedulingType) {
      ...
      if (
        ...
        throw new AuthorizationError("User does not have permission to create this new event type");
      }
    ...
    if (ctx.user.organizationId && !ctx.user?.organization?.isOrgAdmin && !teamId) {
      ...
      if (orgHasLockedEventTypes) {
        ...
        throw new AuthorizationError(
          "User does not have permission to create this new event type - organization has locked event type creation"
        );
      }
    }
    ...
    try {
      ...
      if (e instanceof PrismaClientKnownRequestError) {
        if (e.code === "P2002" && Array.isArray(e.meta?.target) && e.meta?.target.includes("slug")) {
          throw new ValidationError("URL Slug already exists for given user.");
        }
      }
      throw new ValidationError("Failed to create event type");
    }
  } catch (error) {
