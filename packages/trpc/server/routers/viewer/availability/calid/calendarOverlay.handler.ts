...
import type { TCalIdCalendarOverlayInputSchema } from "./calendarOverlay.schema";

type CalIdCalendarOverlayOptions = {
  ctx: {
    user: NonNullable<TrpcSessionUser>;
  };
  input: TCalIdCalendarOverlayInputSchema;
};

export const calIdCalendarOverlayHandler = async ({ ctx, input }: CalIdCalendarOverlayOptions) => {
  const { user } = ctx;
