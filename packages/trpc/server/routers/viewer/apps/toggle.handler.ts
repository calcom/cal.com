...
export const toggleHandler = async ({ input, ctx }: ToggleOptions) => {
  ...
  const app = await ctx.ctx.prisma.app.upsert({
    where: {
      slug,
    },
    update: {
  ...
  if (!enabled) {
    ...
    if (
      ...
      const appCredentials = await ctx.ctx.prisma.credential.findMany({
        where: {
          appId: app.slug,
        },
        select: {
      ...
      const eventTypesWithApp = await ctx.ctx.prisma.eventType.findMany({
        where: {
          metadata: {
      ...
      Promise.all(
        eventTypesWithApp.map(async (eventType) => {
          ...
          await ctx.ctx.prisma.eventType.update({
            where: {
              id: eventType.id,
            },
            data: {
