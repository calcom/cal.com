...
import type { TCalidListTeamAvailaiblityScheme } from "./calidListTeamAvailability.schema";

type GetOptions = {
  ctx: {
    user: NonNullable<TrpcSessionUser>;
  };
  input: TCalidListTeamAvailaiblityScheme;
};
...
async function buildMember(member: Member, dateFrom: Dayjs, dateTo: Dayjs, teamName?: string) {
...
async function getInfoForAllTeams({ ctx, input }: GetOptions) {
  ...
  const teamIds = await prisma.calIdMembership
    .findMany({
      where: {
        userId: ctx.user.id,
      },
    ...
    .then((memberships) => memberships.map((membership) => membership.calIdTeamId));
  ...
  >`SELECT COUNT(DISTINCT "userId")::integer from "CalIdMembership" WHERE "calIdTeamId" IN (${Prisma.join(
    teamIds
  )})`;
...
export const listTeamAvailabilityHandler = async ({ ctx, input }: GetOptions) => {
  const { cursor, limit, searchString } = input;
  const teamId = input.calIdTeamId || ctx.user.organizationId;
  ...
  if (!teamId) {
    ...
    if (!isMember) {
      ...
      totalTeamMembers = await prisma.calIdMembership.count({
        where: {
          calIdTeamId: teamId,
          ...(searchString
            ? {
  ...
  const buildMembers = teamMembers?.map((member) =>
    buildMember(member, dateFrom, dateTo, member.calIdTeam?.name)
  );
