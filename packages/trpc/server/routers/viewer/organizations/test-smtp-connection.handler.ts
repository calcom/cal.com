import {
  ...
  getSmtpService,
} from "@calcom/features/di/smtp-configuration/containers/smtp-configuration";
...
import type { TTestSmtpConnectionInput } from "./test-smtp-connection.schema";
...
export const testSmtpConnectionHandler = async ({ ctx, input }: TestSmtpConnectionOptions) => {
  ...
  if (!user || !password) {
    ...
    const storedConfig = await configService.getDecryptedConfigForOrg(organizationId);
    if (storedConfig) {
