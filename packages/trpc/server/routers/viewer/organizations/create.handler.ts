...
export const createHandler = async ({ input, ctx }: CreateOptions) => {
  ...
  let orgOwner = await ctx.prisma.user.findUnique({
    where: {
      email: orgOwnerEmail,
    },
  });

  const hasAnOrgWithSameSlug = await ctx.prisma.team.findFirst({
    where: {
      slug: slug,
      parentId: null,
      isOrganization: true,
  ...
  if (!isOrganizationConfigured) {
    ...
    const instanceAdmins = await ctx.prisma.user.findMany({
