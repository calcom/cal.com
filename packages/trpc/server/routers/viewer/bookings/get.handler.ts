  ...
  const queriesWithFilters = bookingQueries.map(({ query, tables }) => {
    ...
    if (filters?.attendeeEmail) {
      if (typeof filters.attendeeEmail === "string") {
        ...
        const needsJoin = !tables.includes("Attendee");
        if (needsJoin) {
          fullQuery = fullQuery.innerJoin("Attendee", "Attendee.bookingId", "Booking.id");
        }
        fullQuery = fullQuery.where(({ fn, ref, val, eb }) =>
          eb(
            fn("lower", [ref("Attendee.email" as any)]),
            "=",
            fn("lower", [val(filters.attendeeEmail as string)])
          )
        );
