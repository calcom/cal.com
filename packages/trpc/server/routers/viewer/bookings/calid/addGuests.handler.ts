...
import type { TCalIdAddGuestsSchema } from "./addGuests.schema";

type CalIdAddGuestsOptions = {
  ctx: {
    user: NonNullable<TrpcSessionUser>;
  };
  input: TCalIdAddGuestsSchema;
};
export const calIdAddGuestsHandler = async ({ ctx, input }: CalIdAddGuestsOptions) => {
  const { user } = ctx;
  ...
  const isCalIdTeamAdminOrOwner = booking.eventType?.calIdTeamId
    ? await prisma.calIdMembership.findFirst({
        where: {
          userId: user.id,
          calIdTeamId: booking.eventType.calIdTeamId,
  ...
  if (!isCalIdTeamAdminOrOwner && !isOrganizer && !isAttendee) {
