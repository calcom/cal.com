...
const log = logger.getSubLogger({ prefix: ["bookings.calid.get"] });
...
export async function getBookings({
  ...
  filters: TCalIdGetInputSchema["filters"];
  prisma: PrismaClient;
  kysely: Kysely<DB>;
  ...
  const calIdMembershipIdsWhereUserIsAdminOwner = (
  ...
  const calIdMembershipConditionWhereUserIsAdminOwner = {
    some: {
      id: { in: calIdMembershipIdsWhereUserIsAdminOwner },
    },
  };

  const [
    eventTypeIdsFromCalIdTeamIdsFilter,
    ...
    getEventTypeIdsFromCalIdTeamIdsFilter(prisma, filters?.calidTeamIds),
    ...
    getEventTypeIdsWhereUserIsAdminOrOwner(prisma, calIdMembershipConditionWhereUserIsAdminOwner),
    getUserIdsAndEmailsWhereUserIsAdminOrOwner(prisma, calIdMembershipConditionWhereUserIsAdminOwner),
  ]);

  const bookingQueries: { query: BookingsUnionQuery; tables: (keyof DB)[] }[] = [];

  // If user is calid team owner/admin, contain calid team members emails and ids
  ...
  if (!!filters?.userIds && filters.userIds.length > 0) {
    ...
    //  Scope depends on calid team membership:
    // - Throw an error if trying to filter by usersIds that are not within your calid team
    ...
    // 4. Scope depends on calid team membership:
    // - If Current user is CALID_TEAM_OWNER/ADMIN so we get bookings where calid team members are attendees
    ...
    // 5. Scope depends on calid team membership:
    // - If Current user is CALID_TEAM_OWNER/ADMIN so we get bookings where calid team members are attendees via seatsReference
    userEmailsWhereUserIsAdminOrOwner?.length &&
      bookingQueries.push({
        query: kysely
          .selectFrom("Booking")
    ...
    // 6. Scope depends on calid team membership:
    // - If Current user is CALID_TEAM_OWNER/ADMIN, get bookings created for an event type within the calid team
    ...
    // 7. Scope depends on calid team membership:
    // - If Current user is CALID_TEAM_OWNER/ADMIN, get bookings created by users within the same calid team
  ...
  const queriesWithFilters = bookingQueries.map(({ query, tables }) => {
    ...
    // 2. Filter by Event Type IDs derived from CalId Team IDs (if provided)
    if (eventTypeIdsFromCalIdTeamIdsFilter && eventTypeIdsFromCalIdTeamIdsFilter.length > 0) {
      fullQuery = fullQuery.where("Booking.eventTypeId", "in", eventTypeIdsFromCalIdTeamIdsFilter);
    }

    // 3. Filter by specific Event Type IDs (if provided)
    // If both calIdTeamIds filter and eventTypeIds filter are provided, filter 2. ensures the event-types are within the calId teams
...
async function getEventTypeIdsFromCalIdTeamIdsFilter(prisma: PrismaClient, calidTeamIds?: number[]) {
  if (!calidTeamIds || calidTeamIds.length === 0) {
    return undefined;
  }

  const [directCalIdTeamEventTypeIds, parentCalIdTeamEventTypeIds] = await Promise.all([
    prisma.eventType
      .findMany({
        where: {
  ...
  return Array.from(new Set([...directCalIdTeamEventTypeIds, ...parentCalIdTeamEventTypeIds]));
}
...
async function getEventTypeIdsWhereUserIsAdminOrOwner(
  prisma: PrismaClient,
  calidMembershipCondition: PrismaClientType.CalIdMembershipListRelationFilter
) {
  const [directCalidTeamEventTypeIds, parentCalidTeamEventTypeIds] = await Promise.all([
    prisma.eventType
      .findMany({
        where: {
          calIdTeam: {
            members: calidMembershipCondition,
          },
        },
        select: {
          id: true,
...
/**
 * Gets [IDs, Emails] of members where the auth user is calid team admin/owner.
 * @param prisma The Prisma client.
 * @param calidMembershipCondition Filter containing the calid team ids where user is ADMIN/OWNER
