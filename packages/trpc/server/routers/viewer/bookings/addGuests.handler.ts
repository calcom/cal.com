...
export type Booking = NonNullable<Awaited<ReturnType<BookingRepository["findByIdIncludeEventTypeAttendeesUserAndReferences"]>>>;
...
export const addGuestsHandler = async ({
  ...
  const newGuestsDetails = uniqueGuests.map((guest) => ({
    name: guest.name || "",
    email: guest.email,
    timeZone: guest.timeZone || booking.user?.timeZone || "Europe/London",
    locale: guest.language || booking.user?.locale || "en",
  }));
  ...
  if (!evt) {
    throw new TRPCError({
      ...
      message: "Failed to build calendar event",
    });
  }
  ...
  if (emailsEnabled) {
    await sendGuestNotifications(evt, updatedBooking, uniqueGuestEmails);
  }
...
export async function updateBookingAttendees(
  ...
  const repo = bookingRepository ?? new BookingRepository(prisma);

  return await repo.updateBookingAttendees({
    bookingId,
    newAttendees,
    updatedResponses: {
      ...bookingResponses,
