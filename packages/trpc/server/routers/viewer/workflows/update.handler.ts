...
export const updateHandler = async ({ ctx, input }: UpdateOptions) => {
  ...
  const userWorkflow = await ctx.ctx.ctx.prisma.workflow.findUnique({
    where: {
      id,
    },
    select: {
  ...
  if (!isOrg) {
    ...
    const activeOnEventTypes = await ctx.ctx.ctx.prisma.eventType.findMany({
      where: {
        id: {
          in: activeOn,
        },
    ...
    if (userWorkflow.isActiveOnAll) {
      oldActiveOnEventTypes = await ctx.ctx.ctx.prisma.eventType.findMany({
        where: {
      ...
      oldActiveOnEventTypes = (
        await ctx.ctx.ctx.prisma.workflowsOnEventTypes.findMany({
          where: {
            workflowId: id,
          },
          select: {
    ...
    //update active on
    await ctx.ctx.ctx.prisma.workflowsOnEventTypes.deleteMany({
      where: {
        workflowId: id,
      },
    });
    ...
    await ctx.ctx.ctx.prisma.workflowsOnEventTypes.createMany({
    ...
    if (userWorkflow.isActiveOnAll) {
      oldActiveOnIds = (
        await ctx.ctx.ctx.prisma.team.findMany({
          where: {
            parent: {
      ...
      oldActiveOnIds = (
        await ctx.ctx.ctx.prisma.workflowsOnTeams.findMany({
          where: {
            workflowId: id,
          },
          select: {
    ...
    //update active on
    await ctx.ctx.ctx.prisma.workflowsOnTeams.deleteMany({
      where: {
        workflowId: id,
      },
    });

    await ctx.ctx.ctx.prisma.workflowsOnTeams.createMany({
  ...
  userWorkflow.steps.map(async (oldStep) => {
    ...
    const remindersFromStep = await ctx.ctx.ctx.prisma.workflowReminder.findMany({
      where: {
    ...
    if (!newStep) {
      ...
      await ctx.ctx.ctx.prisma.workflowStep.delete({
        where: {
          id: oldStep.id,
        },
      });
      ...
      await ctx.ctx.ctx.prisma.workflowStep.update({
        where: {
          id: oldStep.id,
        },
        data: {
  ...
  if (addedSteps.length) {
    ...
    const createdSteps = await Promise.all(
      addedSteps.map((step) =>
        ctx.ctx.ctx.prisma.workflowStep.create({
          data: {
            ...step,
            numberVerificationPending: false,
  ...
  await ctx.ctx.ctx.prisma.workflow.update({
    where: {
      id,
    },
    data: {
  ...
  const workflow = await ctx.ctx.ctx.prisma.workflow.findFirst({
    where: {
      id,
    },
    include: {
