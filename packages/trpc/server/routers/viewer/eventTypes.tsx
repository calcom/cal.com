...
export const eventTypesRouter = router({
  ...
  get: eventOwnerProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const rawEventType = await ctx.prisma.eventType.findFirst({
        where: {
          AND: [
            {
              OR: [
      ...
      const parsedCustomInputs = (rawEventType.customInputs || []).map((input) =>
        customInputSchema.parse(input)
      );
      ...
      if (eventType.users.length === 0 && !eventType.team) {
        const fallbackUser = await ctx.prisma.user.findUnique({
          where: {
            id: ctx.session.user.id,
          },
          select: userSelect,
      ...
      const currentUserMembership =
        eventTypeObject.team?.members.find((el) => el.user.id === ctx.session.user.id) ?? null;
