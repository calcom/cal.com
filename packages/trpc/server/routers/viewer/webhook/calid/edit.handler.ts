...
import type { TCalIdEditInputSchema } from "./edit.schema";

type CalIdEditOptions = {
  ctx: {
    user: NonNullable<TrpcSessionUser>;
  };
  input: TCalIdEditInputSchema;
};

export const calIdEditHandler = async ({ input, ctx }: CalIdEditOptions) => {
  const { id, ...data } = input;
