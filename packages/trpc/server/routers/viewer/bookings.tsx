...
type UserSelect = Prisma.UserGetPayload<typeof userSelect>;
type IsFixedAwareUser = UserSelect & { isFixed: boolean };
...
export const bookingsRouter = router({
  ...
  event: authedProcedure.input(extendedBookingCreateBody).mutation(async ({ ctx, input }) => {
...
type extendedBookingCreateBody = z.infer<typeof extendedBookingCreateBody>;

async function handler(ctx, input: extendedBookingCreateBody) {
  const {
    recurringCount,
    allRecurringDates,
  ...
  if (!eventType) throw new TRPCError({ code: "NOT_FOUND", message: "eventType.notFound" });
  ...
  try {
    ...
    if (error instanceof BookingDateInPastError) {
      // TODO: TRPCError should not bleed through to the console.
  ...
  if (!users.length && eventType.userId) {
    ...
    if (!eventTypeUser) throw new TRPCError({ code: "NOT_FOUND", message: "eventTypeUser.notFound" });
    users.push(eventTypeUser);
  }

  if (!users) throw new TRPCError({ code: "NOT_FOUND", message: "eventTypeUser.notFound" });
  ...
  let defaultLocationUrl: string | undefined = undefined;
  ...
  if (reqBody.bookingUid) {
    if (!eventType.seatsPerTimeSlot) {
      throw new TRPCError({ code: "NOT_FOUND", message: "Event type does not have seats" });
    }
    ...
    if (eventType.seatsPerTimeSlot <= booking.attendees.length) {
      throw new TRPCError({ code: "CONFLICT", message: "Booking seats are full" });
    }

    if (booking.attendees.find((attendee) => attendee.email === invitee[0].email)) {
      throw new TRPCError({ code: "CONFLICT", message: "Already signed up for time slot" });
    }
  ...
  const userReschedulingIsOwner = ctx.user.id && originalRescheduledBooking?.user?.id === ctx.user.id;
  ...
  try {
    ...
    if (err.code === "P2002") {
      throw new TRPCError({ code: "CONFLICT", message: "booking.conflict" });
    }
    throw err;
  }
  ...
  if (!booking) throw new TRPCError({ code: "BAD_REQUEST", message: "Booking failed" });
