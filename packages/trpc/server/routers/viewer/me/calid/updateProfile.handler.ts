...
type CalIdUpdateProfileOptions = {
  ctx: {
    user: NonNullable<TrpcSessionUser>;
    res?: NextApiResponse | GetServerSidePropsContext["res"];
  };
  input: TCalIdUpdateProfileInputSchema;
};
...
export const calidUpdateProfileHandler = async ({ ctx, input }: CalIdUpdateProfileOptions) => {
  ...
  if (input.completedOnboarding) {
    const userCalIdTeams = await prisma.user.findUnique({
      where: {
        id: user.id,
      },
      select: {
    ...
    if (userCalIdTeams && userCalIdTeams.calIdTeams) {
      ...
      // TODO: Implement calIdTeam event type assignment if needed
    }
  }
...
const cleanMetadataAllowedUpdateKeys = (metadata: TCalIdUpdateProfileInputSchema["metadata"]) => {
  if (!metadata) {
    return {};
  }
