...
export const isAuthed = middleware(async ({ ctx, next }) => {
  ...
  if (!user || !session || session?.user?.isOverlayUser) {
...
export const isOverlayUser = middleware(async ({ ctx, next }) => {
  const middlewareStart = performance.now();
  const { user, session } = await getUserSession(ctx);

  const middlewareEnd = performance.now();
  logger.debug("Perf:t.isOverlayUser", middlewareEnd - middlewareStart);
