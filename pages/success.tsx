                ...
                <div className="fixed z-10 inset-0 overflow-y-auto">
                    <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        <div className="fixed inset-0 my-4 sm:my-0 transition-opacity" aria-hidden="true">
                            ...
                            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-start overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                                <div>
                                    ...
                                    <div className="mt-4 border-t border-b py-4">
                                        ...
                                        <p className="text-gray-500 mb-1">
                                            <ClockIcon className="inline-block w-4 h-4 mie-1 -mt-1" />
                                            {props.eventType.length} minutes
                                        </p>
                                        {location && <p className="text-gray-500 mb-1">
                                            <LocationMarkerIcon className="inline-block w-4 h-4 mie-1 -mt-1" />
                                            {location}
                                        </p>}
                                        <p className="text-gray-500">
                                            <CalendarIcon className="inline-block w-4 h-4 mie-1 -mt-1" />
                                ...
                                <div className="flex mt-2">
                                    <Link href={`https://calendar.google.com/calendar/r/eventedit?dates=${dayjs(date).format('YYYYMMDDTHHmmss[Z]')}/${dayjs(date).add(props.eventType.length, 'minute').format('YYYYMMDDTHHmmss[Z]')}&text=${props.eventType.title} with ${props.user.name}&details=${props.eventType.description}` + ( location ? "&location=" + encodeURIComponent(location) : '')}>
                                        <a className="mx-2 btn-wide btn-white">
                                            <svg className="inline-block w-4 h-4 mie-1 -mt-1" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><title>Google</title><path d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z"/></svg>
                                        </a>
                                    </Link>
