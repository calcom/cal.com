...
const EventTypePage = (props: inferSSRProps<typeof getServerSideProps>) => {
  ...
  const mapUserToValue = ({
    ...
    value: `${id || ""}`,
    label: `${name || ""}`,
    avatar: `${(process.env.NEXT_PUBLIC_APP_URL || process.env.BASE_URL) + "/" + username + "/avatar.png"}`,
  });
                                  ...
                                  <div className="flex items-center mb-2 text-sm" key={period.type}>
                                    ...
                                    {period.type === "ROLLING" && (
                                      <div className="inline-flex">
                                        ...
                                        <select
                                          id=""
                                          className="block w-full py-2 pl-3 pr-10 text-base border-gray-300 rounded-sm focus:border-primary-500 focus:outline-none focus:ring-primary-500 sm:text-sm"
...
export const getServerSideProps = async (context: GetServerSidePropsContext) => {
  ...
  const teamMembers = eventTypeObject.team
    ? eventTypeObject.team.members.map((member) => {
        ...
        user.avatar =
          (process.env.NEXT_PUBLIC_APP_URL || process.env.BASE_URL) + "/" + user.username + "/avatar.png";
        return user;
      })
    : [];
